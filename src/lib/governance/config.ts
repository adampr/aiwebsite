 ...
 * Round 17: names verbosity adoption and the letterhead carry-over, scoped
 * and hedged so nothing here can read as a lie (logos and images are
 ...
 * Round 17c: .docx and PDF letterhead parity (owner ruling). */
export const STYLE_SAMPLE_HELPER =
  `Optional. Upload a policy your organization has the right to share (${STYLE_SAMPLE_TYPES_COPY}). The sample sets the structure: the draft follows its outline, the order its sections run in and what they are called, its heading and list style, and how detailed its sections are, ahead of our own defaults. Section numbering is applied automatically, in the sample's numbering style. A few things are kept even when your sample has no equivalent: anything still to confirm, determination and adoption blocks, signature lines, disclaimers, and version tables. A .docx or PDF sample's page header and footer text also carries into your Word downloads (logos and images are not). Tron is instructed not to reuse the policy's content. Only the extracted text is kept: it is sent to our AI providers with each drafting turn, except the header and footer text, which is used only to build your downloads. Everything is deleted with the project.`;
