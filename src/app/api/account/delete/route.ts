...
// "cascade"); contact_submissions and workshop_interest are personal data
// keyed by email with no FK — removed in beforeDelete.
// governance_usage/governance_meta are global (no per-user rows) and
// untouched.
...
import { contactSubmissions, workshopInterest } from "@/lib/db/schema";
...
export const POST = createAccountDeletionHandler(siteConfig, {
  beforeDelete: async (user) => {
    ...
    await db.delete(workshopInterest).where(eq(workshopInterest.email, user.email));
  },
});
