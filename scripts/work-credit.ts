...
// (§5.16/§5.18). These credits FOLD INTO the Published count on the staff
// lane of the Employee Scorecard (published = cards + live-anchor exhibit
// credits; there is no separate Exhibits column). The lane exists because
// the exhibits on the public Our Work page are page copy rather than
// work_submissions rows: the colleagues who built them were counted by
// nothing, and the builders of the remaining hand-written exhibits read
// "1 published" while the company publicly showcased two or three of their
// tools. History: shipped 2026-08-29, retired by migration 0055 the same
// day, RETURNED 2026-08-31 with migration 0056 on the owner's ask.
//
...
// mapping exists only in the production database, migration 0056 creates the
...
// The eight rows exported on 2026-08-29 before 0055 dropped the table all
// name anchors that have since left the page (one-tool-one-card conversions)
// and are NOT to be re-imported.
//
...
async function main() {
  ...
  console.log(
    "[work-credit] the staff scorecard's Published count reflects this on its next load."
  );
  process.exit(0);
}
