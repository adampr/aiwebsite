...
// §5.18 exhibit credits: shipped 2026-08-29, retired by migration 0055 the
// same day, RETURNED 2026-08-31 with migration 0056 on the owner's ask that
// the hand-written /work exhibits count for their builders). No DB, no fs,
// no env, so scripts/roadmap-tests.ts drives every branch without a
// database.
//
...
// credits against the generated anchor list before folding them into the
// person's Published count, so `lake-house` (which does not exist) is
// indistinguishable from a retired exhibit: the write succeeds, nothing
// errors, and the colleague's Published figure does not move, which is the
// exact bug the table was built to fix. So the id is validated HERE, at the
// write edge, against the same generated list the reader uses.
