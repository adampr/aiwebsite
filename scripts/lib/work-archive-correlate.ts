...
// ---------------------------------------------------------------------------
// The static exhibit lane (ADVISORY ONLY)
// ---------------------------------------------------------------------------
//
// /work renders TWO lanes. Lane A is the DATABASE: work_submissions rows,
// rendered as "From the Team", and the only lane anything above this
// comment can see. Lane B is STATIC: the hand-authored exhibit cards
// written directly in src/app/work/page.tsx (bays 01 to 05) and
// snapshotted into src/lib/work/static-titles.json. A lane B exhibit has
// NO row, NO sha256 and NO bytes anywhere in the database, so a package
// that is fully published as an exhibit is byte-identical, to this tool,
// to a package that was never submitted at all. That is a structural
// blindness, not a bug to be fixed by more hashing.
//
// So the rule for everything below: SURFACE lane B, never DECIDE it. A
// false "already an exhibit" would hide a genuinely missing skill, which
// is worse than the confusion it would spare. Marketing names routinely
// share no word with the skill name (exhibit "TicketScribe" IS the
// `ticket-notes` skill), so name overlap is a QUESTION for the operator,
// never a verdict, and its absence is never evidence of anything.

/** One hand-authored /work exhibit card, as snapshotted from page.tsx into
 * src/lib/work/static-titles.json (GENERATED by
 * scripts/work-static-snapshot.mjs; `bay` is the two-digit bay number,
 * `id` the card's anchor). */
export type ExhibitFacts = { id: string; title: string; bay: string };

/** Words that carry no identity: dropped from BOTH sides before any
 * overlap is counted, so a shared "skill", "tool", "xl" or "it" can never
 * propose a match on its own. File-extension noise is listed too, for the
 * names whose stem stripping leaves one behind. Pinned by test:correlate;
 * "toolkit" and other real nouns are deliberately NOT here.
 *
 * Measured against the 2026-08-29 recovery corpus (165 byte streams) and
 * the 26 exhibits before each addition: "it", "your" and "up" cost no
 * true guess ("IT Support Chicago" is still proposable as
 * {support, chicago}) and "it" alone killed two spurious ones. "ai" is
 * deliberately NOT a stop word: it would leave exhibit "ai.xl.net" with
 * no token at all, a silent blind spot. A new stop word must keep every
 * exhibit self-advisable (pinned by test:correlate). */
export const EXHIBIT_STOP_WORDS: ReadonlySet<string> = new Set([
  "a",
  "an",
  "and",
  "app",
...
/** Drop the trailing extension chain (`x.skill.b64.txt` -> `x`), at most
 * four of them, never everything. */
function fileStem(name: string): string {
  let stem = basename(name);
  for (let i = 0; i < 4; i++) {
    const next = stem.replace(/\.[A-Za-z0-9]{1,5}$/, "");
...
/** Case-folded meaningful words: every run of letters or digits is a token
 * (hyphens, underscores, dots, spaces and punctuation all split), then
 * single characters, pure numbers (copy counters and dates) and stop words
 * are dropped. */
function meaningfulTokens(text: string): Set<string> {
  ...
  for (const t of text.toLowerCase().split(/[^a-z0-9]+/)) {
    ...
    if (EXHIBIT_STOP_WORDS.has(t)) continue;
    out.add(t);
  }
  return out;
}
...
/**
 * Exhibits whose title might be this local file, by shared meaningful
 * token between the exhibit title and the union of (a) the sniffed
 * SKILL.md `name:` and (b) the file's basename stem. At least one shared
 * token is required; results are sorted strongest first (more shared
 * tokens, then the shorter title, then alphabetically).
 *
 * ADVISORY ONLY. A hit is a question ("is this already published under
 * that name?"), never an answer, and an EMPTY result is not evidence that
 * the file is unpublished: the TicketScribe / `ticket-notes` class of
 * match shares no word at all and is undiscoverable here by construction.
 * Callers must say so, and must never let this function move an exit code.
 */
export function possibleExhibitMatches(
  hint: { skillName: string | null; fileName: string },
  exhibits: ExhibitFacts[]
): ExhibitFacts[] {
  const want = meaningfulTokens(`${hint.skillName ?? ""} ${fileStem(hint.fileName)}`);
  if (want.size === 0) return [];
  const scored: { exhibit: ExhibitFacts; shared: number }[] = [];
  for (const exhibit of exhibits) {
    let shared = 0;
    for (const t of meaningfulTokens(exhibit.title)) if (want.has(t)) shared++;
    if (shared > 0) scored.push({ exhibit, shared });
  }
  return scored
    .sort(
      (a, b) =>
        b.shared - a.shared ||
        a.exhibit.title.length - b.exhibit.title.length ||
        (a.exhibit.title < b.exhibit.title ? -1 : a.exhibit.title > b.exhibit.title ? 1 : 0)
    )
    .map((s) => s.exhibit);
}
