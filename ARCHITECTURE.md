...
- **Web intake:** `POST /api/work/submissions/[id]/update` (multipart, same
  ...
  A SUPERSEDED row in "Your submissions" also carries the button when the
  card's live version is NOT in the viewer's own list (statusView
  `currentId`, filled by `liveDescendantId` — a bounded downward walk to
  the published tip): without it, a submitter whose card was last updated
  by someone else had NO surface offering the next update (the "update it
  once and the option is gone" report, 2026-08-04).
