...
returning `{plan: {targets: [{label, directive}], note}}` and logging
`proposal.tron_plan` shape-only; the per-section POST also takes an
optional `directive` (JSON or form field, 600-char cap), the planner's
per-section output round-tripped through the client and re-entering the
revise prompt only through `fenced()`, with `directiveChars` in the
activity meta),
...
sections. A plan turn (`planDocumentRevision`, sessionId `rfpplan_*`, 90s
...
plus one directive each (500-char cap). The client then loops the targets
through the EXISTING per-section revise call sequentially, exactly as
draft-all drafts: one synchronous whole-document rewrite is impossible
under the 100s edge, and parallel calls would starve the shared semaphore.
Nothing writes until a human accepts each proposal (the same PATCH,
staleness-guarded per section). The planner's document text is budgeted
