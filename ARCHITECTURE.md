...
- `local-time.tsx` — `"use client"` `<LocalTime iso={...} [withTime] />`, the sanctioned way to
  ...
  and absolute beyond it, wrapping `when()` in the same seed-then-swap pattern plus
  `suppressHydrationWarning` for the minute-straddle; it is the §5.17 /rfp choice and IS used
  on SSR'd server pages (`/rfp/list`, `/rfp/r/[id]`, `/rfp/knowledge/mine`). Rule: SSR'd
  absolute row → `<LocalTime>`; client-only absolute row → `exact()`; relative-or-absolute
  row → `<When>`. KNOWN GAP (2026-08-25, not fixed): `/rfp/knowledge/review` calls `when()`
  in the SERVER page and passes the string to its island, so the >7-day absolute branch still
  renders the VM's zone wearing no label. It is the same defect class, reachable only through
  the `when()`/`exact()` indirection that a `toLocale*` grep cannot see.
  Values deliberately NOT routed through it keep an explicit pin (only `dates.ts` records the
  rationale in-file): `fmtDate` (`src/components/roadmap/dates.ts`),
  `components/governance/shared.tsx` and `lib/roadmap/platform-copy.ts` pin `timeZone: "UTC"`
  date-only, `/admin/governance` uses the module's business-timezone `fmtDate`, and the §5.19
  `components/requests/serialize.ts` islands receive pre-formatted strings only.
...
expires Y" line renders BOTH halves through `<LocalTime>` (date-only) since
2026-08-25: it is a server component, so both used to resolve to the VM's
...
disagreeing with itself. Suspend/rename/grant/revoke/purge live in
...
`toLocaleDateString`, i.e. the VM's UTC wearing no label): the companies
table's "Created" cell and the governance-doc "added by X, DATE"
parenthetical are date-only (a roster fact and a provenance footnote), while
...
since 2026-08-25, NOT the UTC date-only `fmtDate()` the other step pages
keep, and the admin caption says "titles, status, submission times, and
