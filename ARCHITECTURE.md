...
design), then the company row cascades everything else.
...
refill from ~1072px up. The runway also renders as ORNAMENT (aria-hidden, an
sr summary sentence, no state): the signed-out teaser (node 01 wears the
static up-next invitation) and the staff hub (noInvite - real counts sit on
its cards and a false wayfinding ring would lie; the ornament's hollow
diamonds beside real card counts are the accepted §5.18-unification
trade-off).
...
XL.NET STAFF UNIFICATION (owner ruling 2026-08-08): staff sessions
(isStaffSession: Google + exact-label xl.net, mv NOT required - its
ZERO-CLIENT-AUTHORITY invariant is rewritten in access.ts) get the FULL hub
(`StaffHub`, replacing the old staff-panel explainer) backed by the internal
lane via `staffRoadmapStatus()` (two aggregates, no DNS probe), with card
targets from `STAFF_STEP_HREFS` in config.ts - the ONE staff href map
(governance -> /governance, directory -> /roadmap/scorecard, work ->
/work/submit + a raised secondary link to /work, request/requested ->
/work/requested, scorecard -> /roadmap/scorecard, paid -> /builders
anchors). The (steps) layout admits staff FIRST (before the trusted gate;
`readStaffPage()` is the shared predicate) and renders the shell with
`StepStrip staff` (staff aria-current: scorecard only - hrefs are shared in
staff mode); INVARIANT: every (steps) page must then render a staff variant
(scorecard + scorecard/requests, over `internalScorecard()`) or redirect
staff to its STAFF_STEP_HREFS target (governance, directory, work, request,
requested do) - a page that returns null for staff renders a BLANK shell
because the layout denial screens exist only on the non-staff path. Downloads (`GET /api/roadmap/docs/[id]`): principal-scoped
...
strict emailDomain, scope mapping, §5.19 step-list pins - eight steps,
...
validateRequestBody bounds, status-vocabulary privacy partition). Owner notifications: company created
...
hardening: the INTERNAL lane additionally requires the /rfp provider anchor
(Google). requireWorkUser deliberately admits any-provider xl.net sessions
...
non-Google staff notice explains the provider pin): request form (title,
...
**Scorecard columns** (§5.18 step 07 + staff): `companyScorecard` and the
new `internalScorecard` merge `requestCountsByEmail(scope)` - Requested
...
### 6.y Your AI Roadmap + Requested Work tables (migrations 0035, 0038)
...
# Signed in as staff (Google): /roadmap shows the eight-card staff hub, /roadmap/scorecard
# renders the staff table, and a stale pre-mv staff session must NOT see a blank (steps) shell.
