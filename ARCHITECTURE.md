...
- **11 AI Builder Tools** (`/roadmap/tools`) — 1:N tool cards, each with a
  ...
  step completes with the FIRST tool whose both links are confirmed.
...
moment it is saved, and counts toward nothing until a reachability check
passes. `url_state`/`docs_state` are `unchecked | ok | failed` per field, and
only `ok` may light a step or move the percentage. Editing a URL RESETS that
...
`STAFF_STEP_HREFS` points all three steps at THEMSELVES: unlike governance
there is no public equivalent to redirect to, so a redirect would loop and
...
`{yourWork, percent}`) so a signed-in user pays one fetch per page for both
