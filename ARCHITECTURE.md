...
real state). The hub orientation caption ("Start wherever helps most...")
lives with the HUB callers, not in runway.tsx: the (steps) shell renders
the runway bare. Every runway surface (teaser, both hubs, the shell) wraps
...
ask 2026-08-09): staff sessions (isStaffSession: Google + exact-label
xl.net, mv NOT required - its ZERO-CLIENT-AUTHORITY invariant is rewritten
in access.ts) get the FULL hub (`StaffHub`) backed by the internal lane
...
staff shape so no surface can render a fake verdict). Staff governance is
CONSTANT done (XL.net's governance IS its public offering: the Governance
Builder + published AUP; the card copy says so - flip to a computed count
only if real staff governance docs ever exist). The STAFF DIRECTORY is
...
ONE staff href map (governance -> /governance, directory ->
...
readStaffPage() returns { email, globalAdmin } (globalAdmin via
isGlobalAdminSession, never bare isAdmin; UI affordances only - every
staff WRITE re-derives requireGlobalAdmin). Staff branches on the three
write routes (directory POST, directory/[id] PATCH/DELETE, apollo-import
POST): readStaffPage SELECTS the branch, requireGlobalAdmin AUTHORIZES,
...
(scorecard + scorecard/requests + directory do, over `scorecardRows({
companyId: null })` / the NULL-lane table) or redirect staff to its
STAFF_STEP_HREFS target (governance, work, request, requested do) - a page
that returns null for staff renders a BLANK shell because the layout
...
countPeople in BOTH page lanes, and "/api/roadmap" in proxy.ts). Owner notifications: company created
...
### 6.y Your AI Roadmap + Requested Work tables (migrations 0035, 0038, 0039)
...
| `staff_roadmap_state` | one row (CHECK id=1, seeded by 0039): the staff lane's `apollo_last_import_at/count` - the companies-row stamp has no staff analogue; the write is an UPSERT so a missing row self-heals |
...
# Signed in as staff (Google): /roadmap shows the eight-card staff hub with REAL runway
