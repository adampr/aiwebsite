...
2026-07-29), symlinks skipped, nested zips opaque. Secret scan BEFORE anything
...
admission guards apply. `DELETE .../[id]` — ADMIN-ONLY hard delete (owner
...
- **Web intake:** `POST /api/work/submissions/[id]/update` (multipart, same
  ...
  A SUPERSEDED row in "Your submissions" also carries the button, pointing
  at the card's live version (statusView `currentId`, filled by
  `liveDescendantId` — a bounded downward walk to the published tip):
  ...
  is gone" report, 2026-08-04). Shown UNCONDITIONALLY on superseded rows —
  even when the live row sits in the same list with its own button (owner
  feedback, same day: the original row is where people look for their
  card, so a dedupe there recreates the confusion; both buttons open the
  same update form).
...
role. Four progressive steps: 01 AI Governance doc on file, 02 Company
Directory (Apollo import + manual CRUD), 03 Submit AI-Built Work (the
EXISTING §5.16 pipeline, scoped per company), 04 Employee Scorecard
(derived, published-cards-only).
...
hairline through four diamond nodes; dim / cyan frontier pulse / sand done;
step 04 "Live" never "Done"; segments light when BOTH neighbors done; CSS
only, reduced-motion kills it, aria-hidden visual + visually-hidden <ol>,
state never color-only) + stat strip + step panels, all fed from ONE
...
gets data-gave-up from the DkimStep island when a poll episode ends without
a verdict (selector scoped to .rmp-node--working so a stale stamp is
inert). Teaser node 01 wears the static up-next treatment (shimmer retired;
...
STEP 05 (`key "dkim"`, title "Verified Email", href `/roadmap#step-dkim` - a
hub anchor; it has NO (steps) page, the hub panel opens a native `<dialog>`).
