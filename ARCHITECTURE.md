...
and the bytes stay on the row PERMANENTLY (2026-08-04: Resend's 202 is an
accept, not a delivery; clearing on it destroyed the only copy of two
uploads whose retention emails later bounced — `clearArchiveData` is now an
uncalled ops lever). Attachments go through `toDeliverableAttachment`
...
own SHA-256 separately from the uploaded package's, and states that the
complete upload exists only on the row, retrievable by an operator on the
VM (`npm run work:archive -- <id>`, `scripts/work-archive-export.ts`) with
no second copy today. The rebuilt zip carries
...
- **States:** `pending_approval` = update passed the full panel, parked for
  ...
  retained upload bytes stay on the row like every published row's,
  2026-08-04). In panel.ts the publish exit
...
work_submissions   id uuid PK, user_id uuid NULL FK users ON DELETE SET NULL (published cards
                   ...
                   PERMANENTLY since 2026-08-04 — the owner retention email on publish is
                   an archival copy, never license to clear; excluded from every
                   list/poll/panel SELECT via ROW_COLS),
...
`5056bfd` — post-v1.122, Issue #760 (degenerate summarizer JSON no longer deletes
web-search evidence; see the Last-verified note). The per-version history below runs
