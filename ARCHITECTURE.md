...
of the no-ledger reversal, owner-approved 2026-08-04; since 2026-08-18 the
...
(upload, attach AND remove - attach is member-actionable only on the
company lane); kill switch + rate limits stay in the route files in the
unchanged company order. The STAFF DIRECTORY is
...
| `company_governance_docs` | step-1 documents: upload originals (bytea) or Governance Builder SNAPSHOTS (markdown copy, inert project id, no FK); company_id NULLABLE since 0045 (NULL = the XL.net staff lane, global-admin writes only; every read/write through the required `GovDocScope` param) |
