...
get the CARD REFRESHED and their PACKAGE RETAINED rather than a second card:
the create route 409s a title matching a static exhibit, and a near-miss title
would publish a twin, since `panel.ts` steers a colliding title to a near miss
while `lint.ts` catches only an exact one. New `npm run work:submit`
(scripts/work-submit.ts + pure scripts/lib/work-submit-ops.ts,
`test:worksubmit`) is the scripted twin of `POST /api/work/submissions`: every
gate the route applies after authentication, in the route's own order, held as
data in `SUBMIT_GATES` so it cannot drift, through the route's own
`createSubmission` and `storeArchiveFiles`; it does NOT kick the panel,
because a run started in a short-lived tsx process dies with the operator's
ssh session and `queuedWorkCandidates` already selects exactly the row it
writes. New `npm run work:retain` (scripts/work-exhibit-retain.ts + pure
scripts/lib/work-exhibit-ops.ts, `test:workexhibit`) files an exhibit's own
source archive into the SAME store and ledger with `submission_id` NULL under
`exhibits/<slug>/`, joining the advisory lock `work:backfill` and `work:import`
share. New `npm run work:package` (scripts/work-repo-package.ts) builds a
package from `git archive HEAD` rather than the working tree, dropping every
dotenv-shaped and credential-shaped file and printing each exclusion. NO
schema, NO migration, NO env, NO route change.
Previous: 2026-08-29 §5.20 ADDED IS NOT COUNTING (sections
...
- `checkout-button.tsx` — `"use client"` buy button for a §5.10 offering: POSTs
  ...
  happens on-site.
- `local-time.tsx` — `"use client"` `<LocalTime iso={...} [withTime] />`, the sanctioned way to
  render a STORED instant in the VIEWER's timezone. A server component can only format in the
  server's zone (UTC on the VM), so a bare `toLocaleDateString` there is raw UTC wearing no
  label. Its `useState` seed is UTC-pinned unconditionally, so an SSR'd row and the browser's
  first render emit the same bytes (no hydration mismatch) and a deferred effect then swaps to
  the browser zone; `withTime` adds a clock and a zone abbreviation. Sibling helper `exact()`
  (`src/lib/rfp/time.ts`) formats in the RUNTIME zone on the FIRST render and is therefore for
  CLIENT-ONLY-rendered rows only — using it across a server boundary preserves the mismatch
  rather than closing it. Third lane: `when.tsx` (`<When iso>`) renders relative under a week
  and absolute beyond it, wrapping `when(iso, "UTC")` in the same seed-then-swap pattern plus
  `suppressHydrationWarning` for the minute-straddle. That `"UTC"` argument is LOAD-BEARING,
  not cosmetic: an unpinned seed makes React's eager-state bailout swallow the post-mount
  update and freeze the server's text on screen (mechanism spelled out below). `<When>` is the
  §5.17 /rfp choice and IS used on SSR'd server pages (`/rfp/list`, `/rfp/r/[id]`, `/rfp/knowledge/mine`,
  `/rfp/knowledge/review`). Rule: SSR'd absolute row → `<LocalTime>`; client-only absolute
  row → `exact()`; relative-or-absolute row → `<When>`. **Pick by whether the ROW is
  server-rendered, never by the file's `"use client"` line**: a client component that a server
  page imports statically IS server-rendered by the App Router (there is no `next/dynamic` and
  no `ssr: false` anywhere under `src/`, so nothing escapes SSR that way), and the only real
  escape is a DATA gate — a component whose date branch sits below a `useState(null)` that
  only a mount effect fills renders no date on the server pass. Getting it backwards is a
  defect in BOTH directions: `exact()` on an SSR'd row is a hydration mismatch, `<LocalTime>`
  on a never-SSR'd row paints a frame of UTC on every row. The class was hard to find because
  it hides behind the `when()`/`exact()` indirection, which a `toLocale*` grep cannot see; the
  `/rfp/knowledge/review` gap recorded here as open on 2026-08-25 survived two rounds for
  exactly that reason and was CLOSED on 2026-08-26 (the server page passes `p.createdAt`
  as an ISO prop and the island renders `<When>`, which also unfroze a relative age that had
  been computed at request time and baked into the HTML).
  Since the 2026-08-26 ruling ("every stored timestamp a reader sees renders in the VIEWER's
  zone WITH a clock, deadlines included") almost nothing keeps an explicit pin. What remains:
  `components/governance/shared.tsx` `fmtDate` is still a plain STRING helper, but unpinned and
  with a clock — correct because every one of its 12 call sites is unreachable during SSR
  behind a `useState(null)` gate, and a string is the ONLY thing that works in the three of them
  that sit inside template literals (doc-pane's two footer variants and
  question-pane's confirm blurb); `components/roadmap/dates.ts` `fmtDate` (UTC, date-only) is
  DEAD CODE with zero importers repo-wide, kept only until it is deleted, and its header now
  records the inverted rationale (date-only precision is what makes the zone MATERIAL, because a
  wrong zone flips the whole token silently instead of shifting a visible suffix);
  `lib/roadmap/platform-copy.ts` no longer formats at all and returns a `CheckLine`
  (§5.20); `/admin/governance` no longer uses the module's business-timezone `fmtDate`
  (§5.12); and the §5.19 `components/requests/serialize.ts` islands now receive raw
  ISO instants, not pre-formatted strings. DELIBERATE date-only survivors are now exactly ZERO,
  down from the three this line listed earlier the same day. All three closed on 2026-08-26
  under the owner's "clock on everything" ruling. Two were pages: the `/admin/roadmap` cells
  named in §5.18, which this bullet had recorded as an outstanding exception rather than a settled decision;
  and the pending admin-access row in `admin/roadmap/actions-client.tsx`, whose LAYOUT
  objection was real (it is a compact flex line beside Approve and Deny, and the widening a
  clock adds on the post-mount swap wrapped those buttons onto a second row) and was answered
  by PLACEMENT rather than by dropping the clock — the timestamp moved to the END of the row,
  because flex-wrap only ever displaces what FOLLOWS the item that grew. That is the same rule
  §5.16 `/work/submit` already applies, and the same reason the two `/rfp/r/[id]/workspace.tsx`
  `<When>` spans moved last in their `rfpdoc-actions` rows in the same pass. **When a clocked
  timestamp shares a flex row with controls, the answer is ordering, never precision.**
  The third was not a page at all but a LANE, and it closed in the same pass: the >7-day
  fall-through of `when()` in `lib/rfp/time.ts` used to reach an `ABS` formatter with no hour
  and no `timeZoneName` at all, so EVERY `<When>` surface printed a bare `Aug 9, 2026` once a
  row passed a week, with nothing on screen admitting whose day that was. `ABS` no longer
  exists: it was folded into a single `ABS_TIME` carrying `hour`, `minute` and
  `timeZoneName: "short"`. **`ABS_TIME` backs TWO of the three absolute lanes, not three**:
  `when()`'s >7-day branch and `exact()`, both in `lib/rfp/time.ts`. It does NOT back
  `<LocalTime withTime>`. `src/components/local-time.tsx` imports only React and builds its
  own TWO `Intl.DateTimeFormat`s inline (a UTC-pinned seed and an unpinned post-mount one);
  it reads nothing from `lib/rfp/time.ts` and cannot, since `time.ts` exports formatted
  strings rather than the formatter. What the three lanes actually share is the SAME TOKEN
  emitted from MATCHING option sets (`year`/`month`/`day`/`hour`/`minute` plus
  `timeZoneName: "short"`), which is a real guarantee but a WEAKER one than a single
  formatter, and the difference is operational: editing `ABS_TIME` moves two lanes and
  silently leaves `<LocalTime>` on the old token. Whoever changes the token changes BOTH
  files, and nothing in the compiler or the test suites will say so.
  Do NOT re-pin the dateless `ABS` as intentional design; an earlier
  revision of this bullet recorded it as "the relative-then-absolute lane trades the clock for
  freshness on purpose", and that trade now applies ONLY under 7 days, where the string is
  relative and zone-free ("3 hours ago" is the same sentence in every zone). `when()` also
  gained an optional second argument, `tz`, which PINS the absolute branch to a named IANA zone
  (or "UTC"); omit it and that branch formats in the runtime's own zone, the VM's UTC on a
  server render and the viewer's in the browser. `tz` deliberately does not reach the relative
  branch, which has no zone to get wrong.
  `tz` exists for exactly ONE caller, and the mechanism behind it is worth naming because it is
  expensive to rediscover and invisible in a diff: `<When>`'s `useState` seed is now
  `when(iso, "UTC")`, not `when(iso)`. An UNPINNED seed reads the BROWSER's zone at seed time,
  the deferred effect then recomputes the identical string milliseconds later, and React's
  EAGER-STATE BAILOUT (`objectIs(eagerState, currentState)`) drops the update — no commit ever
  happens. Because `suppressHydrationWarning` only silences the warning and never calls
  `setTextContent`, the DOM keeps the SERVER's text FOREVER, so the component shipped the VM's
  UTC day to every reader while looking like it was doing the opposite. Pinning the seed to UTC
  makes the seed and the post-mount value genuinely DIFFER for any viewer not on UTC, so the
  commit actually happens. The byte-identical hydration this buys is UNCONDITIONAL and is NOT
  contingent on the VM being UTC — the reason `when.tsx`'s header gave for a day, and had
  backwards. React runs the component during SSR, so the server renders THE SEED ITSELF, and
  the seed is UTC-pinned by construction. Measured identical under `TZ=UTC`,
  `America/New_York`, `Asia/Tokyo` and `Pacific/Kiritimati` in two independent jsdom harnesses;
  the same bailout is the reason `<LocalTime>` pins its own seed, and neither may be
  "simplified" back to an unpinned seed.
  CLOSED in the same pass (recorded here because the gap was real for most of a day):
  `src/app/rfp/r/[id]/workspace.tsx` (two sites) called
  `when()` on `sections`, which `useState` seeds from a server prop on an async `force-dynamic`
  page, so both were SERVER-RENDERED. Past 7 days they rendered the VM's zone wearing no label,
  and under 7 days they were a LIVE text mismatch, because `when()` reads `Date.now()` at request
  time and those two bare `<span className="rfpdoc-faint">` had no `suppressHydrationWarning`
  (`<When>` carries one for exactly this reason). Same defect class as a6b52ef, same
  `when()`-indirection reason it outlived three rounds of grepping. Both now render
  `<When iso={sec.updatedAt} />` — `updatedAt` is already a string on every path, so only the
  two spans, their position in the row, and the `when` import changed.
  A FOURTH hiding place, found the same day and worse than the `when()` indirection because it
  needs no indirection at all: a `toISOString().slice(0, 10)` rendered STRAIGHT to a reader.
  `/rfp/knowledge`'s local `day()` helper formatted two timestamptz COLUMNS
  (`rfp_facts.corrected_at`, `rfp_rate_cards.effective_from`) as bare UTC calendar dates on a
  `force-dynamic` SERVER page, so "Corrected 2026-07-27" was Jul 26 at 21:30 for the Chicago
  staffer who made that correction, and the seeded rate card's `effective_from` of exactly
  `2026-01-01T00:00:00.000Z` printed "In force from 2026-01-01" to a reader for whom it was
  Dec 31 **2025** — the wrong YEAR, not merely the wrong day, which is the sharpest available
  illustration of why date-only precision was never the safe option. The idiom matches none of
  the greps that found the rest of the class: not `toLocale*`, not `when()`/`exact()`, not
  `<When>`/`<LocalTime>`. **Grep `toISOString().slice(` too, and triage each hit by whether a
  READER sees it.** Both sites are now `<LocalTime withTime>` behind a local `stamp()` guard
  that returns null for a null or unparseable value: `corrected_at` is NULLABLE, and an Invalid
  Date throws a `RangeError` out of `.toISOString()` and again out of `Intl` inside
  `<LocalTime>`, which on a `force-dynamic` page is a 500 rather than a blank. The
  `.toISOString()` is safe there only because both values arrive through plain drizzle COLUMN
  selects, so `PgTimestamp.mapFromDriverValue` has already made them `Date`s — a raw `sql<>`
  expression skips that mapper and hands back a STRING, which is exactly the trap this round hit
  in `lib/roadmap/db.ts`.
  Values with no viewer at all (emails, generated .docx and standards refs, and filename
  stamps) are outside the class entirely and each build their own UTC string inline.
  `toISOString().slice(0,10)` sites are NOT a category and must never again be triaged as one
  — that lumping is precisely what let `/rfp/knowledge` survive three rounds. Split them by
  destination: a day-bucket DB KEY (a daily counter, a dedupe stamp) is outside the class and
  converting it would silently fork the counter, while the same call rendered into JSX is
  reader-facing and is in the class BY DEFAULT. The test is who reads the string, never which
  method produced it — but read what it decides carefully: it sets the BURDEN OF PROOF, not
  the verdict. A reader-facing UTC timestamp is a defect UNLESS the code at that site carries
  a written, dated rationale for why the viewer's zone is the WRONG answer for that particular
  value; where such a rationale exists the site is SETTLED, and converting it is a regression
  dressed as a fix. **Exactly one such carve-out stands today**, and a value having a viewer
  is not enough to overturn it: `/admin/work#storage`'s `dateLabel` (§5.16) stays UTC with a
  baked `" UTC"` label because it is the archive FILE's store date, which
  `work:backfill`/`work:import` can stamp long after the submission it belongs to, so it is
  not the same fact as the `Submitted … CDT` row a reader compares it against. So a future
  auditor greps this class, then READS the comment at each hit before touching it, and records
  any NEW carve-out in BOTH places (here and in the owning section) — from the diff alone an
  unexplained UTC render and a settled one look identical, which is exactly how a deliberate
  decision gets "fixed" by someone doing their job.
...
- `GET/POST /api/admin/knowledge/refresh` — `createAdminKnowledgeRefreshHandler(siteConfig)`:
  ...
  coordinated with the nightly timer). **FIXED at @aicompany/core v1.106.0 (adopted
  2026-08-28) — the NOTE this entry used to carry is retired.** It read: "the handler
  spawns `<repo root>/scripts/refresh-knowledge.mjs`, which does not exist in this host
  … the manual refresh button spawns a missing file", and it was accurate — the button
  had never once started a crawl here and answered `{"ok":true}` every time, because the
  handler resolved a single repo-root path that exists on no module-as-submodule host.
  `knowledgeScriptCandidates()` (module `src/admin/api/index.ts`) now tries three layouts
  from `process.cwd()` in precedence order — `scripts/`, then
  `packages/aicompany/scripts/`, then `node_modules/@aicompany/core/scripts/` — and the
  SECOND is this host's (`packages/aicompany/scripts/refresh-knowledge.mjs` is present
  here, verified). A miss now returns an error naming every path it looked in, instead of
  reporting success. No host-root shim is needed and none should be added.
...
**The bot-UA filter widened at @aicompany/core v1.107.0 (adopted 2026-08-28), and this
host runs it.** `tracking.botUaFilter: true` in site.config.ts, and `src/proxy.ts` mounts
the module's `createTrackingMiddleware` — so the list is not host-owned and no host code
changed. `BOT_PATTERN` moved out of the module's `tracking/middleware.ts` into
`src/tracking/bot-pattern.ts` (zero imports, edge-safe) because the §19.5 read side now
needs the same tokens as a Postgres `~*` string; `middleware.ts` re-exports it, so every
existing import is unchanged. Fifteen tokens were added to a list that had only ever
caught clients polite enough to name themselves (`bot|crawl|spider|slurp|baidu|yandex|
semrush|ahref|mj12|python|curl|wget`): `node`, `undici`, `bun/`, `axios`,
`go-http-client`, `okhttp`, `java/`, `postman`, `libwww`, `httpclient`, `headless`,
`scrapy`, `http_request`, `-audit`, `-probe`. **Operational consequence for this host,
stated because it is easy to misread as a traffic loss: `page_visits` grows more slowly
from this deploy forward.** Rows already written are untouched — only the §19.5 read side
filters retroactively — so `/admin/analytics`, `/admin/companies` and `/admin/seo`
30-day windows straddle the change for a month. Two tokens are deliberately narrowed and
the reason is the asymmetry that governs the whole list: a missing token admits machine
traffic and is recoverable, an over-broad token deletes real people and is not. `bun/`
carries the slash because bare `bun` is a substring of **`Ubuntu`** and would have
silently deleted every Ubuntu Firefox and Chromium visitor on this host; `java/` is
narrowed on the same reasoning. Upstream states one residual: `node` is the one token
whose safety rests on a device-name argument.
...
**bookable again** (2026-08-26): the next session is **Thursday, September 24, 2026,
8:00 AM to 12:00 PM CT** (CDT, so it starts `2026-09-24T13:00:00Z`), $995 one-time
per seat, capped at 8, sold on **Ticket Tailor** (single seat pool with the
email-invite audience; the July 30 session oversold precisely because site/Stripe and
Ticket Tailor were two separate pools, and August 27 then sold out on Ticket Tailor
alone). The site never sees the checkout.

- **Session module `src/lib/workshop/session.ts`** is the ONE source of truth: the
  labels (`WORKSHOP_SESSION_LABEL` "September 24", `WORKSHOP_SESSION_LONG_LABEL`
  "Thursday, September 24", `PREVIOUS_SESSION_LABEL` "August 27"), the two start
  instants (`WORKSHOP_SESSION_STARTS` 2026-09-24T13:00Z, `PREVIOUS_SESSION_STARTS`
  2026-08-27T13:00Z), `WORKSHOP_TICKETS_URL` (the public Ticket Tailor event page,
  the only place that URL lives in `src/`), `WORKSHOP_PRICE_USD` 995 and
  `WORKSHOP_SEAT_CAP` 8, plus the PURE `workshopWindow(now)` returning one of three
  windows, half-open on each start instant: `"prev-sold-out"` (now < Aug 27 13:00Z:
  August 27 shown sold out ABOVE the bookable September 24), `"booking"` (Aug 27
  13:00Z <= now < Sept 24 13:00Z: September 24 alone), `"tba"` (from Sept 24
  13:00Z: "Next date: TBA", nothing bookable). Pinned by
  `scripts/workshop-session-tests.ts` (`npm run test:workshop`) at each instant and
  one millisecond either side. Changing the date is an edit to this file only.
- **`/builders` workshop card** (`force-dynamic`, per-request `Date.now()`):
  prev-sold-out renders two stacked full-width badge strips (`badge--warn`
  "August 27 · Sold out", no dot, then `badge--light` with the breathing dot
  "September 24 · Booking open"; side by side they overflow the ~412px card
  interior); booking renders the single dotted "September 24 · Booking open" strip;
  tba the dotted "Next date: TBA" strip. In both booking windows the list reads
  "Thursday, September 24 · 8:00 AM to 12:00 PM CT, live on Zoom", the format lines,
  "Capped at 8 attendees · July 30 and August 27 both sold out", and a secondary
  "Can't make September 24?" link to `/builders/notify` (the list emails you when
  the date AFTER it is set); the primary CTA is `<a href={WORKSHOP_TICKETS_URL}
  target="_blank" rel="noopener noreferrer">` "Reserve September 24 · $995" with the
  fine print "One-time · 8 seats · checkout on Ticket Tailor · Zoom link sent at
  registration". The tba window keeps the "Get notified about the next session"
  `/builders/notify` CTA and its "Free to join" fine print. Every branch keeps the
  card at exactly 7 subgrid children (the stacked strips share one wrapper) because
  the two cards share `sm:grid-rows-[auto_auto_auto_auto_1fr_auto_auto]` +
  `row-span-7`, which is what keeps the two CTA buttons on one line at >= 640px.
- **`/builders/notify` is window-aware** through the same module: in the booking
  windows the intro says the next workshop is September 24 (linking
  `/builders#workshop` to reserve) and that the list emails you when the date after
  it is set; in the tba window it says the September 24 workshop has started and the
  list emails you when the next date is set.
...
**Workshop notification list** (added 2026-08-18, the sold-out round; window-aware
copy since 2026-08-26, above):
...
createOpenAiTtsSynthesizer({ apiKey: process.env.OPENAI_API_KEY, voice: "ballad" })`**
(§19.33.2b), used ONLY when the primary cannot produce a chunk. Gemini TTS
refuses particular exact input strings, and one such chunk used to cost an
article its narration FOR LIFE, because the script text never changes so the
render hash never changes so no later nightly retried it. The fallback
re-renders the WHOLE article, so the file is always in one voice; the row's
`voice`/`model_id` record which seam MADE a file — never that a FAILOVER
produced it, so there is no provenance column and a nightly failover is
disclosed only in the run report line and the `[blog-audio]` log series.
`"ballad"` deliberately differs from the primary `"Charon"`: config refuses an
identical `(voiceLabel, modelId)` pair, which would compute the same render hash
and re-ask the SAME vendor. The voice was chosen on a MEASUREMENT rather than on
register (2026-08-25): the module's `contentGate` requires ≥60% of energy in the
200–4000 Hz speech band and 6 of 11 OpenAI voices fail it on timbre alone;
`"ballad"` scores 74–78.5%, where the originally wired `"ash"` passed at 61.5%,
1.5 points of margin, which is not margin for an unattended nightly path. It is
deliberately NOT roleplay's `"coral"`: every host wiring the same fallback would
make the fleet sound identical on exactly the articles that fail over. Re-check
with `tsx packages/aicompany/cli/check-tts-voice.ts` before changing it.
`OPENAI_API_KEY` is host-owned and already present; it stays out of the module
`.env.example` by design.

**v1.105.1 (adopted 2026-08-27): a refusal is TWO classes needing opposite
handling, and the "primary is failing broadly" alarm was answering an
every-article question after ONE article. THIS HOST MAILED THAT FALSE ALARM**
— nightly 2026-08-26, `reported_issues` id 234, on a healthy run; the refused
string was `"Tron's take."`, a section heading present in 48 of this host's 50
articles. That is the whole defect in one line: N articles refusing ONE
per-article TEMPLATE CONSTANT are ONE witness counted N times. Root cause:
v1.103.0 evaluated `fallbackRenders.length >= attemptedRenders` from inside
`runAudioHook`'s PER-ARTICLE branch against run-cumulative counters read
mid-run, so the FIRST fallback of any run satisfied it as `1 >= 1`. Three
changes land here. (1) **The verdict moves to END OF RUN and counts DISTINCT
REFUSED INPUTS, firing at ≥ 2** — it floors the run at WARN, emits the night
token `audio-warn:fallback-systemic`, and mails through `shouldEmailNightly`'s
existing `audio-warn:` PREFIX arm (nothing was added to `OUTAGE_TOKENS`), with
the reason appended to the subject as ` (audio: primary refused N distinct
inputs)`. The line claims only that the primary refused N distinct inputs and
that a fallback pass was ATTEMPTED for the named slugs: `fallbackRenders` is
written BEFORE the content gate, the encode and the store, so an article whose
fallback render was then rejected on timbre published SILENT and is still
counted. `runAudioHook` no longer raises the class per-article, so a successful
failover now reports `warn: false` — always the documented intent (a successful
failover is disclosed, never mailed). (2) **Gemini's `content_blocked` became a
bounded-retry class** (the same 3 attempts everything retryable gets), where
v1.103.0 gave every non-auth, non-429 4xx exactly ONE attempt. The classes are
told apart by the vendor's own `error.code` in the BODY, never by status; a
body that is missing, unparseable or code-less keeps the pre-v1.105.1
status-only mapping. **The retired claim this replaces is the one this document
used to make:** "refuses exact strings DETERMINISTICALLY — 15/15 on an HTTP 200
with zero output tokens, 3/3 on an HTTP 400 'Model tried to generate text'".
Those two measurements are TRUE of the two strings they were taken on and do NOT
generalise to the class — and they are not even the same class. The
zero-output-token 200 is `empty-generation` and the "Model tried to generate
text" 400 is `invalid_request`/`bad-request`; both remain NON-retryable and keep
their single attempt. The POLICY refusal (`content_blocked`) is a third class
and is NOT deterministic: the same bytes refused and then succeeded across ~43
identical calls on 2026-08-27 (2 refusals pooled; a control string drew 0 in
25). What that licenses is a DIRECTION, not a rate — any rate quoted for it is
an order of magnitude, not a property of the vendor. The counter-evidence is
honoured rather than deleted: a roleplay chunk on 2026-08-12 refused three times
running with the same policy code, so some blocks genuinely ARE persistent;
hence a BOUNDED retry and a failover that still happens, at a cost of 2 extra
calls plus backoff on a persistent block. No OpenAI policy-refusal spelling is
matched on the fallback seam — this fleet has never captured one, and guessing
it would let an unmeasured string set retry policy. (3) **The liveness canary
`probePrimaryAlive` now uses the module's own `ttsClassRetryable`** instead of a
second hand-maintained copy, and retries, so a single `content_blocked`,
`rate-limit` or `transient` blip no longer convicts the primary and fails the
article; `auth`, `bad-request`, `empty-generation` and a null/short-PCM answer
still refuse the failover on the FIRST ask, and a primary refusing every attempt
still yields `primary-down`. Expect FEWER failovers, and so fewer articles in
the fallback voice; **how many fewer is not known and is not claimed**, because
the attempts are not independent. **Evidence:**
`packages/aicompany/reviews/2026-08-27-blog-fallback-systemic.md` §17 backtests
56 real host-nights across 2 hosts, of which only 6 were fallback-capable: the
OLD rule fired twice, both FALSE POSITIVES and both in production (this host's
id 234 and roleplay's id 162); the NEW rule fires 0 times. That proves
SPECIFICITY and leaves **SENSITIVITY UNTESTED** — no night in the corpus had a
genuinely broken primary. Two limits are structural and are not bugs: the rule
is unreachable on a host rendering ONE article a night (failover is
whole-article, so a run needs ≥ 2 articles), and it over-counts on per-article
INTERPOLATING templates, where one underlying trigger yields a different
120-char excerpt per article. Do NOT "close the sensitivity gap" by OR-ing in
distinct CHUNK INDICES: the same string lands at different indices when intro
lengths vary, which reintroduces exactly the false positive this release
removes. Patch release: NO schema, NO env, and nothing under `deploy/` — the
rendered `deploy/deploy.sh` template hash is unchanged at the new pin, so there
is no `render.mjs` re-render. `@breezystack/lamejs` (pure-JS MP3 encoder) became a direct
...
403344e — the tip of the module's `master`, one commit past the `v1.107.2` tag
(`git describe` reports `v1.107.2-1-g403344e`; `upgrade:check` correspondingly
reports `module version 1.107.2 (package.json (untagged checkout))`). Crossing
this pin takes FOUR releases at once — v1.106.0, v1.106.1, v1.107.0, v1.107.1,
v1.107.2 — because the `fix/knowledge-crawl-body-read` fork merged into master
here. **NO schema, NO env, NO `site.config.ts` edit. ONE deploy template
re-render — see below.**

*What the detector did.* It compared raw pageviews across two 7-day windows and
mailed `WARN blog traffic: down N% week over week` below a −20% threshold. Raw
pageviews are not readers: one machine client hammering a blog path for a few
minutes becomes a baseline week that the following, healthier week cannot match.
The counted population is now DISTINCT SESSIONS with non-browser UAs excluded, in
BOTH window expressions. Session identity is
`COALESCE(session_hash, 'nohash:' || id::text)` rather than a bare
`COUNT(DISTINCT session_hash)`, because `COUNT(DISTINCT col)` discards NULLs
silently and `session_hash` is nullable — an ungroupable row counts as exactly ONE
session, which preserves the old `COUNT(*)` behaviour for precisely the rows that
cannot be grouped. That direction is chosen deliberately: the failure mode is
over-counting sessions (an alert that does not fire), never under-counting them (a
phantom collapse mailed to an operator). `user_agent` NULLs are KEPT — absence of a
UA is not evidence of a bot, and a beacon that stopped recording UAs must not read
as a collapse. **The motivating evidence is upstream's, measured on roleplay
production, and is NOT re-measured here**: 236 vs 424 raw pageviews (−44%) over the
same two windows in which distinct sessions went 204 vs 110 (+85%), the baseline
week carrying one self-inflicted burst — 2026-08-17, 172 `/insights` hits from 11
sessions, 123 of them non-browser.

*What changes for THIS host's numbers, and what does not.*
`blog.traffic.extraPrefixes` — the v1.107.0 knob that lets a host count a blog the
module does not own — is deliberately **UNSET here**, and site.config.ts has no
`traffic:` block. This host runs ONE blog (`blog.types[0].urlPrefix = "/blog"`) and
has no second blog to opt in; the knob defaults to `[]`, which makes
`blogTrafficPrefixes(config)` return exactly `derivedBlogPrefixes(config)` — the
pre-v1.107.0 path set, verified at the adopted pin, not assumed. So the counted
POPULATION here is byte-identical to before and only the counting RULE moves.
Expect the §19.5 numbers to be SMALLER in absolute terms in both windows from the
first nightly after this deploy. That is the definition, not a traffic loss, and
the `/admin/seo` panel and the nightly mail move together by construction (one
`readBlogWow`). If an open `blog-traffic-wow` episode auto-resolves,
`describeWowResolution` now states WHICH it was: `readBlogWow` also returns a
`legacy` pair — the same rows scored under the pre-v1.107.0 rule, computed in the
SAME single query so both pairs describe the same rows and the same `now` — and the
note says "resolved by the DEFINITION CHANGE, NOT by a traffic recovery" rather
than letting a redefinition wear a recovery costume.

*The bot list.* Root cause was upstream of the detector: `BOT_PATTERN` caught only
clients polite enough to name themselves. It moved to the module's
`src/tracking/bot-pattern.ts` and widened by fifteen tokens — full list, the
`bun/`-vs-`Ubuntu` trap, and this host's ingestion consequence are recorded in §5.6
"Page-view tracking", because `tracking.botUaFilter: true` here means the widening
narrows what this host WRITES as well as what §19.5 COUNTS. The two filters are
needed separately (widening ingestion cannot un-write rows already inside tonight's
windows) and sharing one constant is what stops them drifting.

*Merged in at the same pin — the knowledge crawl (§8), and it is substantive here.*
This host runs the module crawler nightly (`aiwebsite-knowledge.timer`), so
v1.106.0's containment lands: a stalled response BODY is now a counted fetch error
instead of a dead night (the `AbortSignal.timeout` handed to `fetch()` governs the
body stream too, and only the headers phase sat inside a `try`, so one
`await res.text()` rejection propagated out through `Promise.all(workers)` and took
the whole run down); a run losing more than `max(3, 10%)` of the pages it attempted
now ABORTS and keeps yesterday's knowledge, because all three sinks REPLACE.
**Operator-visible: this host may now receive a FAILED knowledge email on a night
that previously produced a silent, truncated OK. That is the fix working, not a new
breakage.** Also fixed here: the "Run knowledge crawl now" button, which had never
started a crawl on this host and reported success anyway — see §5.6's
`/api/admin/knowledge/refresh` entry, whose standing NOTE this release retires.
Upstream states one KNOWN GAP it did NOT close: a crashed crawl is still invisible
to the issue ledger and the staleness WARN is self-cancelling.

*Re-render — REQUIRED, performed, and verified rather than assumed.* Not because
v1.107.x changed a template, but because this pin now contains v1.106.0, whose
`setup-vm.sh.tpl` change this host had never crossed. Exactly ONE of the 16
rendered files moved: `deploy/setup-vm.sh`, stamp
`setup-vm.sh.tpl@c998badfbc6e…` → `@b5631580f01f…`, which equals the sha256 of
`packages/aicompany/deploy/templates/setup-vm.sh.tpl` at the new pin (checked
directly). The other 15 re-rendered byte-identically. The content change is the
deploy liveness sentinel: it now ARMS only if the site answered healthy on
`http://127.0.0.1:3000/api/health` BEFORE staging (`APP_PORT=3000` and
`src/app/api/health/route.ts` both confirmed present here). On a FIRST deploy
nothing has ever listened on that port, so every probe missed, three misses arrived
in 30 s, and the sentinel SIGKILLed the staging it exists to protect — observed on
topmspnearme as exit 137 with an empty `/var/www`. **Strictly narrowing on an
established host like this one: the probe succeeds, the sentinel arms, and
behaviour is unchanged.** No local patch was overwritten — the diff is the upstream
template change and nothing else. **ORDER IS LOAD-BEARING and was followed**: pin
bumped FIRST, `node packages/aicompany/deploy/render.mjs` SECOND, rendered file
committed WITH the gitlink. `deploy.sh` compares each rendered stamp against the
CURRENT submodule's templates, so rendering before the bump stamps the old hash and
fails the drift gate on deploy. Re-rendering does not re-run `setup-vm.sh`.

*Gates run here (verbatim in the adoption commit message).* `npx tsc --noEmit` exit
0 with no output — **this host has no `typecheck` npm script**, so `npm run
typecheck` exits 1 as a MISSING SCRIPT and must not be read as a type error.
`npm run config:check` → `config:check OK`, exit 0. `npm run upgrade:check` → exit
0, module version 1.107.2, stamped version in DB 1.98.0, 19 pending entries. **That
backlog is a reporting artifact, not outstanding work**, and it grew from 13 to 19
for a knowable reason: five new releases plus `v1.100.1`, an entry that exists only
on the merged branch line and was absent from MIGRATIONS.md at the old pin. The
merge was checked for loss on the axis that can be checked mechanically — the set of
`## v1.*` headings at 8b4c842 minus the set at 403344e is EMPTY, and v1.100.1 is the
gain. The re-render entries the tool lists (v1.102.0, v1.104.x) were applied here
previously (6ebc28a, 29db1f6) and the stamp simply lags the repo.

*Residual, recorded rather than resolved here.* The module's own MIGRATIONS.md
preamble states "Releases soak on the canary host (aiwebsite) for 3 days before
other hosts bump" — **this host IS that canary**, and this pin was adopted as part
of a fleet-wide rollout that moves the other hosts on the same day rather than
after a soak. The departure is a rollout decision, not a property of the release;
it is written down here so that a later reader does not infer a 3-day soak
happened. Two things make the departure worth naming rather than waving through.
First, the §19.5 numbers step to a new unit on EVERY host at once, so if the new
definition misbehaves there is no host still on the old rule to compare against.
Second, this release line has a documented track record of exactly the failure a
soak exists to catch: v1.107.1 shipped a correctness REGRESSION to `/admin/seo`
that v1.107.2 exists to fix (per-group `COUNT(DISTINCT session)` summed as if
session counts partitioned, which they do not), and the v1.107.2 note states in its
own words that it is "the fifth claim in this release line that did not
reproduce", the pattern in all five being "a release note describing what the
author believed rather than what was run". Read this host's own gate output above,
not the release notes, for what was actually verified here.
...
(heading lines are the AUTHORITATIVE channel and decide alone whenever they
produce a winner; body lines are consulted only when no heading line carries a
marker at all, sub-numbers and body-line letters never vote, and a winning
style needs >=2 matching lines) and the renderers format the
...
**Round 21 (2026-08-26) — the sample outranks our defaults, plus four defect
fixes (owner: "make the sample governance document create constraints that are
the highest priority").** (a) *Precedence.* The FORMAT SAMPLE block's closing
absolute "Where the sample conflicts with the RULES or the DOCUMENT ALLOWLIST,
the sample loses" is REPLACED by one explicit ladder: the sample is the
AUTHORITY on structure (topic order, which sections exist and what they are
called, internal organization, heading/list style, table usage, definitions
style, document-control layout, section length), and a named LOSES tier that
still outranks it (DOCUMENT ALLOWLIST, host-owned numbering, the STANDARD
REFERENCE with the no-invented-identifier and no-borrowed-citation rules, the
stringency rule, the FFIEC DRAFTING RULES wherever they appear, and the
required elements: `[TO CONFIRM]` markers, determination/adoption blocks,
signature lines, disclaimers, version tables). "If matching the sample's
section length would force omitting required content, completeness wins"
becomes "add the smallest section that holds it rather than dropping the
content". One ladder, one owner: three separate designs proposed three
incompatible ladders and two of them silently dropped the FFIEC rules from the
protected tier, so a test now asserts "FFIEC DRAFTING RULES" appears inside the
ladder and that "the sample loses" appears nowhere. (b) *Verbosity ceiling
450 -> 600 words/section* (`VERBOSITY.targetCeilWords`, prompt.ts). NOT
uncapped, and deliberately so: the stated target must stay inside the ENFORCED
per-section cap or the model is told a length it cannot legally emit. At ~7.2
chars/word 600 words is ~4320 chars against `CAPS.sectionMarkdownMaxChars`
6000, and three such sections still fit `CAPS.turnOpMarkdownMaxChars` 16000.
The turn-zero ceiling stays 300 (it protects the 24k group budget). Raising
further requires raising those caps AND the repair pass with them (the repair
call must re-emit a whole corrected turn inside its own timeout), which is a
separate round. (c) *Sample headings stop being an instruction sink
(security).* Bucket titles for `adopt_outline` were the one piece of untrusted
document content leaving the `<<<SAMPLE` fence, interpolated into a
double-quoted list with no escaping, in instruction position next to the RULES.
A heading carrying `"` could close the list and continue as forged rules. The
fix is the FENCE: the list now travels inside its own `<<<SAMPLE_TITLES` block,
one `- Title` per line, so a quote is inert data. `sanitizeOutlineTitle`
(prompt.ts) is deliberately MINIMAL beside it, defanging only what could break
out of that fence (control characters, so a title can never span lines, and
`<<<`/`>>>` runs), and keeping the pre-existing 80-char slice. It must stay
minimal: these strings are NOT prompt-only, since `applyOps` stores the matched
title on the section and both renderers display it, so anything stripped here
is stripped from the customer's delivered document. A first draft that also
removed quotes and apostrophes and capped at 60 chars shipped
"Employees obligations" for "Employee's obligations" and cut real headings off
mid-word; that is now a test pin. The document-title dedupe in
`sampleOutlineTopTitles` compares LIKE WITH LIKE (both sides sanitized): while
it did not, the sample's own document title leaked into the bucket allowlist
and rendered as a top-level heading of the generated policy.
(d) *D1: edit-again carries the format sample.* `createImportedProject`
(governance/db.ts) seeded the reopened project with NO `style_sample_*` at all,
so a document brought back from the roadmap file permanently lost its numbering
style, length target and letterhead. The edit route now reads the SOURCE
project via `styleSampleForProject` (no ownership check: the caller already
proved lane access to the doc row, and edit-again exists for the colleague who
did not create the draft) and passes it through. It DOES carry the same
`retentionCutoff()` term as every other project read here, and that term is
required rather than defensive: sweeps are periodic, so without it the read
would resurrect an expired-but-unswept sample into a fresh 30-day life after
the UI had already promised it was deleted. The sample is deleted with the
project exactly as `STYLE_SAMPLE_HELPER` promises, and nothing here resurrects
it. No migration.
(e) *D2: the snapshot round trip no longer splits sections.* `projectMarkdown`
writes each section as `## <title>` and `parseSnapshotMarkdown` reads EVERY
`## ` line as a boundary, so a drafted section carrying its own `## ` inner
headings split into several: an 11-section AUP came back as 16 to 20 sections
with every ordinal moved and blueprint ids lost. `demoteSectionHeadings`
(snapshot.ts) now shifts each section's headings so its shallowest is `###`,
making `## ` mean boundary and nothing else. Render-neutral
(`normalizeSectionBlocks` rebases each section's depth to its own shallowest
level), idempotent, and byte-safe for stub restore (no blueprint placeholder
contains a heading, verified across all 127). Chosen over an in-band marker
deliberately: any marker line is body text to the CURRENT parser, so a head
marker invents a phantom document that consumes the first blueprint slug and
loses every stub flag, and a trailing marker block lands inside the last
section's stored markdown. This scheme has no rollback edge (an old server
parses a new snapshot correctly, and better than it parses today's). Snapshots
already on file keep today's behavior until a confirm refreshes them.

Two constraints on the demotion, both found by executing the first draft of it
and both load-bearing. (i) THE CEILING IS THE RENDERER'S, NOT MARKDOWN'S:
`parseMarkdown` parses `#`..`####` only, so a heading pushed to `#####` stops
being a heading, fails `breaksParagraph`, and glues into the paragraph above as
literal hash characters in the doc pane and the .docx alike, permanently (the
next pass sees min>=3 and never shifts it back). The shift is therefore
ALL OR NOTHING: a section that cannot clear `## ` without passing `####` (three
heading levels starting at `#` or `##`) is returned byte-identical and keeps
today's split, because today's split beats a destroyed heading. (ii) FENCES ARE
NOT SKIPPED, and `parseSnapshotMarkdown` stays FENCE-BLIND. `parseMarkdown` has
no fenced-code concept, so a `# retention in days` line inside a fence already
renders as a heading everywhere in this system; demoting it is consistent with
how it displays, and leaving it would put a `# ` line in the body for the
parser to read as a new DOCUMENT. Making the parser fence-aware instead was
tried and reverted: its fence state is per FILE while the emitter's is per
SECTION, so one unbalanced fence made every later `## `, `# ` and `---`
opaque and collapsed a 7-document FFIEC file into one section, including for
rows already on file. Test-pinned with an odd fence count. KNOWN RESIDUAL,
pre-existing and deliberately not touched this round: `parseSnapshotMarkdown`
also reads a bare `---` line as a DOCUMENT separator, and nothing demotes or
escapes one written inside a section body (a horizontal rule), so that route
into a mis-split snapshot is still open. Closing it means either rewriting a
body's bytes or making the separator ambiguous, and this round already proved
how expensive a wrong move here is; it wants its own round with the same
executed round-trip evidence. (f) *D3:
heading recovery keeps the sample's own marker.*
`recoverLeadingNumberedHeadings` (style-sample.ts) hardcoded `## N. `, so a
sample reading `1) Purpose` was rewritten to `1. Purpose` and a correct "paren"
detection was laundered into "decimal", silently, on exactly the PDF class the
recovery exists for. It now re-emits the chain's own separator (one separator
per chain: a single outline has one marker shape). (g) *D4:* the "heading lines
weigh 3x" claim above was stale; there is no weighting.
...
snapshot row. Since the same-day edit-again round the snapshot is no longer
frozen once the source project's 30-day retention removes the original either:
§5.18's `POST /api/roadmap/docs/[id]/edit` seeds a new review project from the
on-file markdown and re-keys the row's `governance_project_id`, so confirm-final
keeps refreshing the one on-file document indefinitely. Pure decision table + all copy in
...
**Timestamps (2026-08-26):** every stored instant on the page goes through one
local `<Stamp>` component wrapping `<LocalTime withTime>`, which is the ONLY
client JS the page loads. It replaced two helpers that disagreed with each
other: `ts()` wrapped the module's `@aicompany/core/admin/format` `fmtDate`
(business timezone, self-labelling) purely to render null as "n/a" instead of
the em-dash glyph the host copy ban forbids, while `dateOnly()` printed "Aug
25" in the same business zone with no year, no clock and NO ZONE LABEL, so
the one column a reader could not sanity-check was also the one that told
them least. Departing from the module convention is the point and follows the
a6b52ef `/admin/work` precedent: the module formats in the BUSINESS's zone
because it treats a timestamp as a fact about the company, and the ruling is
about the person reading the row. Converting only `dateOnly` would have been
worse than converting neither, since two columns of one table would then have
carried two different zones. `<Stamp>` keeps both inherited guards on purpose
— null returns "n/a", an unparseable value returns itself — because
`<LocalTime>` throws a `RangeError` out of Intl on an Invalid Date. The
relative `deleteCountdown()` ("in 12d" / "sweeping") is day-granular and
zone-immaterial, and is untouched.
...
2026-08-27: `time-saved.ts`, the one pure parse/format module behind the
self-reported "time saved per month" figure, imported by client components
and routes alike).
...
validation (**NO kind field since 2026-08-28** — the route reads no `kind`
from the body and ignores one posted under that name; the package decides,
see "Kind inference" below — title 4-60, blurb 0-900 — **no minimum since
...
from OAuth; optional `timeSavedHours` through `parseTimeSavedHours`, 400
`invalid_request` carrying the parser's own sentence — it is checked
immediately after the blurb, the first line at which the value exists, which
cannot spare the upload itself (`req.formData()` has already buffered it)
but does spare `inspectArchive`) → archive inspection (`extract.ts`, all in memory, bytes never
...
call — the brain is already on a submission's critical path and a second
inference on the UPLOAD path would mean watching a spinner while a model picks
the radio button you were not shown) returning `{kind, rule, reasons, note?}`.
Rungs, first hit wins: (1) `bare_document`, no package at all → skill; (2)
`claude_code_project`, a `.claude`/`.claude-plugin` directory at any depth, a
`.mcp.json`, or `.github/workflows/` → program; (3) `skill_package`, the
upload's extension is `.skill`/`.ski` → skill; (4) `program_scaffolding`, an
arch-doc basename, `CLAUDE.md`, or a `.bat`/`.cmd`/`.exe`/`.msi` launcher AT
THE PACKAGE'S OWN ROOT → program; (5) `wrapped_skill_package`, EXACTLY one
inner `.skill`/`.ski` → skill; (6) `skill_document`, ANY `.md` at the package
root carrying `name:`+`description:` front matter → skill; (7)
`skill_document_weak`, a file named `SKILL.md` → skill; (8)
`program_dependencies`, a dependency/build manifest at depth ≤2 → program; (9)
`program_source`, source outside `scripts|bin|tools` → program; (10)
`sole_document`, exactly one non-boilerplate `.md` at the package root →
skill; (11) `wrapped_archive`, exactly one inner archive of any kind → skill;
(12) `default_program`.
**"At the package's own root" is not "depth ≤1"** (rung 4, rung 6, rung 10).
Read literally, depth ≤1 means "the root or one folder down", and in a package
with NO wrapper folder that makes every first-level SUBDIRECTORY count as the
package's own: a Skill's `references/design.md` would fire rung 4, call the
whole thing a program, AND then be resolved as the required architecture
document, so the card would be written from a reference note. `wrapperFolder()`
therefore accepts depth 1 only when EVERY path shares that same first segment,
which is what "one wrapper folder" always meant.
**THE ORDER IS THE DESIGN, and three placements are load-bearing.** Rung 2
outranks rung 3: the extension records how a file was exported, `.claude`
records what it is. Rung 4 outranks rungs 5, 6 and 7, because a Claude Code
program very often CONTAINS a skill while a CoWork Skill never carries an
architecture doc at its root, a launcher, or project instructions —
containment points one way only, and putting the SKILL.md rung above it is
exactly the mistake the humans made. But the dependency manifest is NOT in
rung 4: it sits at rung 8, BELOW the Skill-document rungs, because a Skill
that ships helper scripts ships their `requirements.txt` with them, and
convicting it on that file while `HELPER_DIR` deliberately exempts the scripts
themselves was self-contradictory (it would have hard-refused a shape the
production corpus contains). Rungs 10 and 11 exist for the same reason in the
other direction: they are PREDICTIONS of what extract.ts's Skill ladder will
do (it resolves a sole non-boilerplate `.md`, and it opens a single inner
archive one level down), and calling those shapes programs here would
manufacture a hard 422 for packages the next stage accepts. Note rung 11 must
cover an inner `.zip`, not only a named `.skill`: `walkLevel` collects
`.skill` AND `.zip`, so the documented wrapper shape (2026-07-30, "a wrapper
.zip holding the .skill and its .md must work") reaches extract.ts's lazy open
only if the classifier sends it down the Skill lane. Where the Skill lane is
wrong the cost is a SOFT doc-missing the standalone upload can rescue; where
the program lane is wrong the cost is a hard refusal, so the ladder leans that
way on purpose.
`kindVerdictSentence()` composes from `reasons` (noun phrases, joined after
"because it has") or from `note` (a clause, for the rungs that decide on an
absence or on a fact about the upload); gluing the two shapes together
produced "because it has no Skill document and no program scaffolding was
found", broken English offering the evidence AGAINST its own verdict on the
one rung whose reading most needs to be arguable. Rung 4's arch-doc test is deliberately NARROWER than `matchesArchDoc`
(no README-with-Architecture-heading): that widening belongs to the doc
RESOLVER, which runs after the kind is settled, not to the classifier, where
it would sweep in Skills whose README explains how the Skill works.
Validated against all 85 production rows on 2026-08-28: 82 agreed with the
stored kind and 3 flipped skill→program (see the reclassification script
below). `inspectArchive(bytes, kind|null, {packageName})` takes null to infer
and a non-null kind to PIN (the update lane only, where a card's kind belongs
to the card); it always computes the verdict, so a pinned lane can still
disclose that the package disagrees. `ExtractOk` carries `kind` (the one to
store) and `kindVerdict`; `ExtractErr` carries both optionally, set on every
...
the archive is still walked EXACTLY ONCE (`zipMaxEntries` and
`corpusInflateTotalMaxBytes` accumulate across levels), the lazy inner OPEN
stays strictly downstream of the decision (classification reads inner archives'
names, count and declared size, never their contents), `finish()` sorts a COPY
of `state.texts` (both ladders resolve by `.find()` in walk order), and the
program ladder is restricted to outer-level texts (`matchesArchDoc` measures
depth on the DISPLAY path, so `wrapper.skill!/architecture.md` would pass its
depth gate). That restriction reads a `TextFile.inner` flag stamped at walk
time, NOT a `"!/"` string test: `normalizePath` accepts a directory segment
ending in `!`, so a real outer entry can be `Project!/architecture.md` and a
string test would have excluded a document the program lane must see. Required-doc rule (owner
...
depth ≤1. The package (field `file`, .skill/.zip, ≤100 MB) is required on both
kinds; the standalone reviewed document (field `skillMd`,
≤1 MB) is OPTIONAL since 2026-07-30 and, since 2026-08-28, is offered to and
accepted from EVERY submitter rather than Skills only — the form cannot know
what the package is, so refusing a program's separately attached architecture
doc would refuse a file the site itself invited. The wire name `skillMd` is
historical and deliberately unchanged (renaming it would move the form, both
routes and the email lane in lockstep for a label nobody sees); `md_*` is
therefore NO LONGER NULL on program rows, and `notify.ts` labels the retention
email's hash line from `md_name` instead of the literal "SKILL.md".
**Standalone-document rescue, made symmetric (2026-08-28):** a Skill's
doc-resolution failure has always been rescuable by the second upload; the
program ladder hard-failed instead, so a program whose architecture doc sat
beside the zip was a dead end. Both routes now rescue: on
`missing_architecture_doc`/`doc_too_short` with `extracted.kind === "program"`
and a valid standalone attached, a SECOND `inspectArchive` PINNED to `"skill"`
runs (the skill ladder never hard-fails on doc resolution, so it yields the
manifest/corpus/hashes the `ExtractErr` does not carry), and the row is stored
with the PROGRAM kind from the FIRST pass — the pin is a means, never a
result. One extra inflate, paid only by a package that was otherwise about to
be refused outright and only when a `.md` is actually attached. A failure from
the rescue pass (a corrupt inner `.skill`, credentials inside it) still
refuses, and REPLACES the missing-document refusal rather than being
swallowed by it: a credential found inside a bundled archive has to be
answered with the rotate-and-resubmit message, not with "your zip needs an
architecture document". A rescue supplies a missing document, it never
launders an archive. TWO CONSEQUENCES of pinning the Skill ladder are cleaned
up before the row is written: the rescue result's manifest and corpus are
filtered to OUTER-LEVEL entries (`outerLevelOnly`), because the Skill pass
opens a nested archive the program lane never opens and storing
`outer.zip!/inner/file` rows would break extract.ts's own "kind program never
opens nested archives" invariant, classify.ts's `KindSignals` contract and the
reclassification script's assumption, besides feeding the panel text from a
bundle the program lane never read; and an inner-archive failure from that
...
standalone ALWAYS outranks the package's own document for either kind,
because the person chose that exact file in a field whose help says it wins;
by email the `.md` is merely whichever markdown attachment `pickSkillDoc`
...
**`scripts/work-kind-reclassify.ts` (`npm run work:reclassify -- [--apply]
[--id <uuid>] [--allow-partial]`, 2026-08-28, owner directive "go back over
all the submits as some were mislabeled as CoWork but were Claude Code").**
Re-decides the KIND of every historical row through the SAME
`classifyWorkKind` ladder the upload path uses, so old rows and new ones carry
labels that mean the same thing. THE DATABASE IS ENOUGH and nothing is
inflated, downloaded or parsed: `file_manifest_json` (the full path list of
the walk) and `corpus_files_json` survive on published rows, so a row whose
original was cleared after its retention email classifies exactly like one
that still holds its blob. INTAKE PARITY is the contract: `packageName` is
`archive_name` (`""` and never null when absent, since null is the
bare-document rung that short-circuits the ladder to skill), `paths` and
`texts` EXCLUDE every `"!/"` entry (inner-archive paths are recorded only
after the lazy inner open, which runs strictly after classification, so
feeding them back would reach a verdict intake could not), and
`innerArchivePaths` uses `walkLevel`'s own collection rule (`.skill`/`.zip` at
depth ≤1, NOT classify.ts's wider `.ski` test) with a `.ski` at depth ≤1
DISCLOSED as worth a human look. Rows whose stored evidence is provably
partial (a manifest sitting on the 300-entry cap, or a `SKILL.md` the corpus
carries no text for when the verdict landed on `program_source`/
`default_program`) are reported but NOT written without `--allow-partial`.
DRY RUN IS THE DEFAULT. `--apply` writes exactly two things: `kind`, and the
reviewed document's text moved to the column the new kind names
(`architecture_text`/`skill_md_text`) — inert today because `panel.ts` reads
them through a coalesce, done so a future reader keying off `kind` does not
find NULL in the column its own label points at, and falsifying nothing
because the document's real filename stays in `corpus_files_json`. It writes
NOTHING else: not `card_json` (this corrects the LABEL, not the review — the
card was written from the documents actually submitted, and re-reviewing is
`npm run work:rerun`, a separate act with a fresh published_at and re-fired
emails), not `md_*` (they record that a second file WAS uploaded, and the
store's slot 01 ledger reconciles against them), not `corpus_files_json`, and
NOT `updated_at` (on this table that column is a retention timer
`sweepExpiredWork` reads, so bumping it would silently grant every failed and
received row another month of life because an operator fixed a badge). Each
row is written in its own transaction whose WHERE re-states the kind the scan
read, so a row that changed underneath the run is reported as raced rather
than overwritten; idempotent, refuses to run as root, exit 1 on any
failed/raced row. FIRST RUN 2026-08-28 against production: 85 rows, 82 agreed,
3 flipped skill→program — "Outage Detective" (an `architecture.md` over a
Python `src/`+`tests/` tree), "Follow-up Email Drafter App" (an
`architecture.md`, two `package.json`, a `start.bat`) and "Printer Port
Reconfiguration Utility" (a `Run.cmd` launcher). All three carried a
`SKILL.md` somewhere in the tree, which is exactly why a human reading their
own upload called it a Skill. Four other zip-packaged rows that LOOK
mislabeled by title ("ISP Outage APp", "ISP Outage Checker App", "SOQL /
Report Request Translator", "License renewal tracking") are genuine Skills by
content and were correctly left alone; the two "App" titles are the clearest
argument for deciding from files rather than names.
...
[--file <path>] [--md <path>] [--extra <name>=<path>]... [--force] [--yes]`,
at least one of --file/--md/--extra) files an EXTERNALLY recovered original
(admin-mailbox retention attachments, Resend's inbound store, and since
2026-08-29 the "AI Builders Skills and Code (XLnetters)" Slack canvas) into
the store, creating ledger rows so the console manages it like any new file.
Recovery lane for BYTE-LESS rows ONLY (refutation F1): it refuses any row
still holding `archive_data` OR `md_data` (existence-bit probe; a
bytea-holding row's store copy must come from `work:backfill`, the row's own
bytes, never an outside file). The ledger gate is per SLOT (2026-08-29;
before that ANY ledger row refused the whole row): for each slot the run
would write, a LIVE ledger row at that slot refuses (already managed, no
silent double-import) and an admin-DELETED row at that slot refuses too
(deliberate cleanup, final at slot granularity; `work_archive_rel_path_uq`
is a full unique index and manual SQL stays the only override). Slots the
run does not touch are left alone, so a row whose only ledger row is slot 00
(the 08-19 backfill shape) accepts `--md`; the refusal names every
conflicting rel_path with its state, and a ledger rel_path without the
`NN-` prefix `storedRelPath` mints refuses loudly as a tampered ledger rather
than being ignored (pure `importSlotRefusal` + `ledgerSlot`, unit-tested).
SHA-256 verification is settled over ALL --file/--md files BEFORE any write
through the pure, unit-tested `importShaRefusal` gate, unchanged: local
package hash must equal `archive_sha256`, local --md hash must equal
`md_sha256`; a mismatch refuses with both hashes printed, `--force`
overrides with a loud PROVENANCE UNVERIFIED warning in the console output
ONLY (the ledger schema is unchanged, records the file's own sha256; the
sha-hardened clearing transaction means a forced import can never cause a
wrong-file bytea clear), and a row with no recorded sha proceeds and says
so. Files store slot-explicit under the row's stamped
`archive_name`/`md_name` (the local basename is transport junk; a
standalone `--md` goes to slot 01 under `md_name`, gated on `md_sha256`).
`--extra <name>=<path>` (repeatable) stores an ASSOCIATED file: something
that travelled beside the submission but is NOT the recorded original
upload (the canvas recovery had two: a differently-built
`license-renewal-tracker.skill` on a row that records `files (2).zip`, and
the real `kb-style-guide.md` on a row that recorded `kickoff-agenda.md` as
its md, mis-attached at submission). Extras take the lowest free slots
>= `EXTRA_SLOT_MIN` (02; free = no ledger row live OR deleted, pure
`freeExtraSlots`) under the given name through `sanitizeStoredName`, carry
NO originality claim (they never enter `importShaRefusal`; nothing on the
row is theirs to match, so `--force` does not apply to them; the ledger
simply records the file's own sha256 like every other file), and the
console labels each one `ASSOCIATED FILE (not the recorded original)` so a
slot-02 file is never read as provenance for the row. Names must be bare
filenames (no `/` or `\`) that keep a real extension after sanitizing and
are unique within one run; `.b64.txt` names are refused with a decode-first
message because the canvas carries base64-armored `.skill` files and the
store holds artifacts, not their transport. The byte-less rule applies to
extras too (an associated file cannot be filed against a row that still
holds its original bytes; that row's store copy comes from `work:backfill`
first). Prints the rel_paths + ledger ids of the rows THIS run created,
matched against a pre-run ledger read so a pre-existing row with the same
name and length can never stand in for a failed write, and lists already
stored files separately; touches nothing else on the row (no bytea, no status, no UPDATE
of any kind). `--yes` skips the confirm prompt (`work:rerun` precedent).
Pure pieces (`planRowBackfill`, `byteLessRowClass`, `parseImportArgs`,
`extraNameRefusal`, `importShaRefusal`, `importSlotRefusal`, `ledgerSlot`,
`freeExtraSlots`, the lock key and slot constants) live in
`scripts/lib/work-archive-ops.ts` and are unit-tested DB-free in test:work
...
import-with-eyes-open, never claimed as the original); screened-only when
...
**Exhibit archives (2026-08-29)**: `/work` renders two lanes of cards and only
one of them has rows. The hand-authored exhibits in `src/app/work/page.tsx`
(bays 01 to 05) carry no `work_submissions` row, so every submitted package had
a durable copy in the archive store while the packages behind the exhibits were
retained nowhere. `npm run work:retain -- --exhibit "<exact card title>" --file
<package.zip> [--doc <document.md>] [--dry-run] [--yes]`
(`scripts/work-exhibit-retain.ts`, pure pieces in
`scripts/lib/work-exhibit-ops.ts`, `test:workexhibit`) files an exhibit's own
source archive into the SAME store and the SAME `work_archive_files` ledger
with `submission_id` NULL. NO schema change and NO migration: the column was
already nullable and already `SET NULL` on submission delete, and `title` /
`file_name` were already snapshots, so the ledger row keeps its meaning with
nothing new to store. Paths are `exhibits/<slug>/<NN>-<sanitizedName>` from
`storedExhibitRelPath` (`archive-naming.ts`), the slug derived from the card's
EXACT title by `exhibitSlug`, which REFUSES rather than inventing a name for a
title reducing to no `[a-z0-9]` characters; that prefix can never collide with
a submission directory, which is always a DB-minted uuid. `--exhibit` must
match a title in the generated `static-titles.json` exactly (near matches are
named), the run takes the advisory lock `work:backfill` and `work:import`
share so the three can never interleave, and re-filing is honest per SLOT: a
byte-identical file at a ledgered slot SKIPS on a sha match, a different file
at a live slot REFUSES, and a slot whose ledger row was admin-deleted stays
retired (the `rel_path` unique index is FULL, so admin cleanup is final for
scripted lanes exactly as it already is for import). Any refusal refuses the
RUN, not the slot: a half-filed exhibit is the worst state to inherit. Unlike
the intake path, `storeExhibitArchive` REPORTS failure instead of swallowing
it, because intake can swallow only on the strength of the row's bytea still
being a copy and here there is no other copy. `/admin/work#storage` labels a
null-submission row as an exhibit archive rather than as "submission removed",
which it was previously and falsely saying. There is deliberately NO retrieval
path: `work:archive` is submission-keyed, so getting an exhibit archive back is
a VM file copy.

WHY THE TWO LANES STAY SEPARATE, now that an exhibit can carry both a retained
artifact (here) and a named builder (`work_static_credits`, §5.18), which is
most of what a row carries: an exhibit must NOT become a row. The editorial
...
<address>] [--attribution <FirstName>] [--time-saved <hours>] [--dry-run]
[--yes]` (`scripts/work-submit.ts`, pure pieces in
`scripts/lib/work-submit-ops.ts`, `test:worksubmit`) is the scripted twin of
`POST /api/work/submissions`, in the family of `work:transfer` and
`work:import`. It exists because the owner submits his own repositories: the
web form needs a browser session no headless lane can mint, and an email
attachment caps out far below what a repository weighs. Every gate, band,
message and ladder step is the route's own function in the route's own order
(the quota runs THIRD, ahead of every title gate, and `SUBMIT_GATES` holds the
order as data so a reorder or a drop fails the suite), through the route's own
`createSubmission` and `storeArchiveFiles`, so the row is indistinguishable
from one the site created. Deliberately not reproduced, each with its reason in
the file header: session auth, CSRF, the in-process rate limiter, the
Content-Length precheck (all properties of an HTTP request that does not exist
here) and `brainHealthy` (the route checks it because it kicks synchronously
two lines later; this lane does not kick, and refusing here would discard a
good row over a self-resolving condition). The LANE check the session gate
implies IS reproduced: `companyId` is null and `--email` must be in
`WORK_SUBMIT_DOMAINS`, so it cannot write a row the site could not have
produced; a company's private lane is refused outright. No panel kick and no
...
another session's half-built change and a package should be the committed
project. On top of what `.gitignore` already excludes it drops every
...
lifting optional directive lines `Kind: CoWork Skill|Code program` (SINCE
2026-08-28 a DISCLOSURE, never a decision: `inferKind` is DELETED and the
package decides through `classifyWorkKind`. The line is still parsed, still
lifted out of the blurb and still answered in the receipt, and it still
REJECTS in exactly one place, an update whose line names the other kind, where
the card pins the kind. The extension gate that refused a `.skill` package
against a Code program card is also GONE, in lockstep with the web update
route: classify.ts rung 2 is the standing refutation of its reasoning, since a
`.skill` holding an agent-configured repository IS a program, and the card's
kind is pinned by the row rather than by the file extension. Its old rule — `.skill` or a standalone `.md`
attachment → skill, bare `.zip` → program — was wrong twice over: the typed
line outranked the files, and "a .md is attached" silently reclassified every
Code program that mailed its architecture doc beside the zip. The `.md`
attachment is now picked on every lane, because the old pick was gated on a
provisional kind that was itself computed from the presence of a `.md`, so
the two were one answer) and `Credit: <first name>` (same validation as the form; never
...
**Per-stage recovery (2026-08-25).** `WORK_CAPS.brainTurnTimeoutMs` is
150 s. Derivation, not a guess: the measured worst legitimate stage on
2026-08-25 was 75_587 ms, so the old 90 s left 14.4 s of margin on a normal
day, and 300 s is undici's un-raisable `headersTimeout` on `callBrain`'s
plain-fetch path, above which any value here is silently inert. A longer wait
must therefore use the re-attach seam, never a bigger number.

The seam lives in exactly ONE place, `runPanelInner`'s `call()`;
`callPanelBrain` stays one dispatch, which is what keeps `title-infer.ts`'s
20 s lane untouched. One recovery attempt per ARMED stage, drawn from a
...
(a ledger refusal needs the day to roll). A timeout older than
`cacheRetentionMs - 300_000` is not re-attached either, because past that
horizon the brain's prompt cache is certain to miss and a re-POST would start
a second fully billed generation.

`reattachBrainTurn` and `BRAIN_PROMPT_CACHE_RETENTION_MS` come from
`@aicompany/core/brain/client` (pinned v1.104.2; the `reattachBrainTurn`
contract itself was introduced in v1.103.2 and is unchanged by the v1.104.x
deploy-gate releases; the module dependency sits
beside the §7 brain contract and the host must never edit the submodule to
suit itself). It re-POSTs the envelope BYTE-IDENTICALLY, exactly once, never
as a poll: a cache miss is wire-indistinguishable from an attach, so N probes
would start N generations. It rides `node:http`, where the caller's budget IS
the ceiling, which is the only reason a 600 s wait is real. The ledger unit
is spent BEFORE the POST and is never refunded on a replay, which makes
`work_usage` an upper bound on spend, the safe direction for a budget guard.
Measured on this box on 2026-08-25: a 625.1 s turn the site had abandoned at
its ceiling was replayed by a same-promptId re-POST in 6 ms.

**Heartbeat pump.** While a single stage call is in flight, `panel_heartbeat_at`
is written every `panelBeatIntervalMs` (45 s), so a long legitimate wait is
not mistaken for a deploy orphan. The pump is COUNT-BOUNDED by
`panelBeatBudget()`, derived from that stage's own worst-case budget, and the
bound is load-bearing rather than tidy: `panel_heartbeat_at` is the only
liveness detector the panel has (`anotherPanelRunning`,
`queuedWorkCandidates`), so an unbounded pump on a promise that never settles
would keep the column fresh forever, every `kickPanel` would refuse `busy`,
`drainAction` maps `busy` to `stop`, and no row in any lane would run again
until pm2 restarts. Bounded, a hung stage stops beating and goes stale in
`panelStaleMs` (240 s) exactly as before. Clearing in a `finally` is NOT
sufficient, because a `finally` never runs if the awaited promise never
settles. `heartbeatPumpSafe()` pins that one missed beat still cannot orphan
a live row, and the pump is a hard precondition of the recovery seam: a 600 s
wait without it would push the heartbeat past `panelStaleMs`, the drain would
offer the live row as an orphan, `claimPanel` would grant a new attemptId, and
a second panel would run against the same row.

**Panel failure contract.** `failPanel()` returns whether the row actually
flipped and NULLs `panel_heartbeat_at`. The return value is the caller's gate:
zero rows means the `eq(status,'running')` predicate produced the no-op it
exists to produce (a published row whose post-publish side effect threw and
reached `runPanel`'s catch), and a no-op must never mail or open an issue.
The NULL is what makes Retry work immediately; without it `claimPanel`'s
non-fromHeld staleness gate refused every Retry for up to `panelStaleMs` after
a failure, a flat four minutes of an inert button. `panel_error` now carries
plain submitter-safe prose from `panelFailMessage(stage, reason)` with NO
machine tag (`view.ts` projects it verbatim to the submitter and `/admin/work`
renders it raw), and it names the step with the same `WORK_STAGE_LABELS` label
the progress line uses, so the failure line and the progress line can never
call one step two names. `PanelFailReason` is one union: `timeout`,
`transport`, `parse`, `budget`, `no_document`, `crash`. All six fail sites
route through ONE `failRun()`, which calls `notifyPanelFailed()`
(`work/notify.ts`) on a real flip: an operator WARN mail with a stable subject
distinct from every held subject (the §5.12 bounce ledger keys on the
subject), a `reported_issues` row keyed `work-panel:fail:{reason}:{lane}`
(EPISODIC, bounded at 6 reasons x 2 lanes = 12 rows ever, so it cannot evict
the 500-row triage window), and a lane-branched SUBMITTER mail. That third
terminal notification, alongside published and held, is what makes
`EMAIL_PROMISE` true, and company copy never names Adam, `/admin` or
`/work/submit` (`FAILED_NEXT_STEPS`, asserted by `test:work`).

**Poll contract.** `statusView()` projects `stage` (the RAW stage name, never
a composed sentence), `stageIndex`, `stageCount`, `elapsedMs` (the RUN's age
while running, from `panel_started_at`; the ROW's age while received),
`serverNowMs`, `waiting`, `slow` and `queueReason`. `slow` is derived from the
stage START the beat pump records, never from `panel_heartbeat_at`, whose age
no longer measures progress now that the pump refreshes it every 45 s. `GET
/api/work/submissions/[id]` reads the 16-column narrow projection through
`submissionListRowById()` rather than `submissionById()`, because the tracker
polls it from the dialog and `ROW_COLS` drags `corpus_files_json`, both doc
texts, the transcript and `card_json` on every tick; DELETE keeps
`submissionById`, which needs the full row. Both GETs attach the queue-wait
reason: process-local, ROW-KEYED, 180 s TTL (`work/queue-signal.ts`), with a
`deployBlocksPanel()` fallback for the post-cutover case where a restart wiped
the stamp. It is memory and not a column deliberately: the reason expires in
about a minute and no migration can be justified for it. `WORK_POLL_MS` is
10 s and every poll pauses on a hidden tab, refreshing once on return.

**Live progress surfaces.** `src/app/work/submit/review-progress.tsx` is the
ONE tracker, in two modes: parent-fed (`row`, used by the `/work/submit` list,
which already polls) and self-fetch (`id`, used by the submit dialog and the
roadmap page). Three slots: a single-sentence `role="status"` live region, a
nine-segment decorative bar, and a per-second clock that is anchored to the
server's `elapsedMs` rather than to a client clock. ALL NINE segments fill on
ANY terminal status, so a held or failed row never leaves empty segments
asserting a review that never finished, and the live sentence switches to
`workTerminalLine(status, lane)` (or the failure prose), so the last running
sentence is never left frozen on screen. No percentage and no ETA: stages 8
and 9 are conditional, so a clean run publishes after 7. The per-second clock
sits OUTSIDE the live region and the rounded-minute phrase INSIDE it, so a
screen reader hears about 15 updates a run rather than 900 or one per stage.
No focus management anywhere in the component. SURFACE PARITY, stated so it
need not be discovered: `/work/submit` and the submit dialog on both `/work`
and `/roadmap/work` carry live progress; the roadmap In Review list carries it
on the THREE NEWEST active rows (the cap is the `work:poll` limiter, 30 per
60 s per user against 6 polls a minute per row) and nothing else on that page
auto-updates.

**Queue drain cadence.** 15 s boot tick (`QUEUE_BOOT_DELAY_MS`), 60 s steady
cadence (`QUEUE_TICK_MS`), and after a pass that ends in a GLOBAL stop, one
15 s fast re-tick (`QUEUE_FAST_RETRY_MS`) up to 8 consecutive
(`QUEUE_FAST_RETRY_MAX`), which is what closes the post-cutover dead minute a
restarted interval used to cost. `PANEL_PASS_TAKEOVER_MS` moved to
`work/config.ts` and is 45 minutes, pinned by `test:work` against
`panelWorstCaseRunMs()` (about 34.75 min): at 30 minutes a HEALTHY worst-case
run would log an incident-shaped "pass stuck" line. The drain still never
re-runs a FAILED row. Re-examined on 2026-08-25 and UPHELD:
`panelRunsPerSubmissionPerDay` is 3 and `claimPanel` enforces it per UTC day
OUTSIDE the fromHeld branch, so two automatic retries would consume the row's
entire manual recourse and both failure emails would then instruct a Retry
guaranteed to be refused; a backoff also lands inside the same nightly-blog
turn that caused the failure, and brain-api registers no disconnect handler,
so an abandoned dispatch keeps generating and adds to the contention it is
waiting out. In-run recovery plus `notifyPanelFailed` replaced it.
...
only the columns `statusView()` projects (16 since the 2026-08-25 round added
`panel_started_at`, which is what makes the tracker's elapsed clock measure
the RUN rather than the row age on a row that waited in the queue first;
15 before that, when the transfer round added the owner triple
`submitter_email`/`creator_email`/`company_id`), cap
...
covers received/stale automatically; failed stays manual-only).
`POST .../time-saved` — the "afterwards" half of the time-saved ask
(2026-08-27, its own block below): `requireWorkUser()` rather than
`requireXlUser()` because BOTH /work/submit and the company /roadmap/work
list offer the editor, 20/60 s on `work:timesaved:{userId}`, body `{hours}`,
owner-or-`verifiedWebAdmin` behind the same single 404, deliberately
status-BLIND (unlike retry/update/transfer: the published row is the main
case, since the figure is usually learned after the tool ships). No kill
switch: that switch pauses panel SPEND, and editing a self-reported number
costs nothing while pausing it would strand a wrong figure on a live card. `DELETE .../[id]` — ADMIN-ONLY hard delete (owner
...
ambiguous. Since 2026-08-25 the row's submitted-at renders through a labelled
`Submitted <LocalTime … withTime/>` in the ADMIN's own timezone (the label
because the chip beside it can read "published", and a bare clock there reads
as the publish date); it used to be
`createdAt.toISOString().slice(0,16)`, i.e. bare UTC wearing no zone label at
all. This is a deliberate departure from the module's admin convention
(`@aicompany/core` `admin/format.fmtDate` formats in the BUSINESS timezone and
returns an em-dash glyph for null, which the host copy ban forbids) — the
directive is about the person reading the row. The `#storage` ledger's
`dateLabel` below is deliberately NOT converted: it is the archive file's
store date, which `work:backfill`/`work:import` can stamp long after the
submission, so it is not the same fact. That ruling stands, but since
2026-08-25 the value carries an explicit `" UTC"` suffix appended
server-side, because a bare `2026-08-19 14:07` sitting ~110 lines under a
`... CDT` submitted-at row read as a local clock hours off (five, for the
Chicago reader in that example). The label
goes on the VALUE, not on a column header (the storage rows are bare spans
in a flex line and have no headers) and not in the section prose (a
paragraph away from the thing it disclaims); it is a string change only, so
`StorageFileView`'s prop shape is untouched.

**Time saved per month (2026-08-27, owner ask: "allow anyone that submitted
work to either at submission (optionally) or afterwards under their
submissions to submit a new field Time saved per month for you").** One
nullable column, `work_submissions.time_saved_minutes integer` (migration
0049: `ADD COLUMN IF NOT EXISTS` per the 0044/0046/0048 precedent, plus the
migration-only range CHECK `work_submissions_time_saved_ck` 1..44640 wrapped
in `DO $$ ... EXCEPTION WHEN duplicate_object`, since ADD CONSTRAINT has no
IF NOT EXISTS and drizzle cannot model CHECKs at all — `drizzle-kit push`
would silently drop it, which is one more reason deploys run `migrate`).
NULL means NOT REPORTED, which is the honest state of every pre-existing row
and of every row that arrives by email intake (that lane is deliberately
untouched: the owner adds the figure afterwards on the row). Entering 0
hours CLEARS back to NULL, so "nothing reported" has exactly one storage
spelling and a wrong figure on a live card is always removable. 44640
minutes is 744 hours, every hour of a 31-day month: the physical ceiling on
time saved WITHIN a month, and the refusal copy names it so a typo like 6000
is diagnosable.

The human types HOURS and the database stores whole MINUTES. That split is
the reason `src/lib/work/time-saved.ts` exists and is PURE (no DB, no
session, no `node:*`): the submission form, both row editors, the create
route and the time-saved route import the same `parseTimeSavedHours`, so a
value the form accepts and a value a route accepts cannot drift, and a
single server-only import in that file would drag a DB driver into a browser
bundle. `parseTimeSavedHours(raw)` takes `string | number` only — refused BY
TYPE otherwise, because `Number(true)` is 1 and `Number([])` is 0 and both
would reach a published card looking like a deliberate report — trims,
treats absent/empty/whitespace/0 as `{ok:true, minutes:null}`, refuses
negatives, non-finite values and anything over `TIME_SAVED_MAX_HOURS`, and
otherwise stores `Math.round(hours * 60)` clamped to at least 1 (0.005 h
rounds to 0 minutes, and storing THAT as NULL would tell a submitter their
report vanished; 1 minute is also the CHECK's floor).
`formatTimeSavedPhrase` renders the card prose ("6 hours 30 minutes a
month", singular/plural correct for both units) and returns null for
not-reported so every surface prints NOTHING rather than a zero, which on a
card would read as a claim that the work saves no time.
`formatTimeSavedCompact` is the table cell ("6h", "6h 30m", "45m", bare "0").
`hoursFieldValue` pre-fills an editor at two decimals. The original reason
(a `step=0.25` input rejects its own pre-filled value when it carries the
float tail of `minutes / 60`) is gone with the grid, but the rounding stays
for a plainer one: `minutes / 60` puts "6.500000000000001" in a field a
person is being asked to CORRECT, and two decimals is exactly the precision
that round-trips back through `parseTimeSavedHours` unchanged, which is the
property `npm run test:work` actually pins.

Two write moments, both belonging to the SUBMITTER. (1) At intake: the
shared `<SubmissionForm>` (the /work dialog, /work/submit and the company
/roadmap/work dialog are one component, so all three lanes gained it at
once) renders an optional `Time saved per month for you (optional)` number
input between the description and the package upload — it is a fact about
the work, not about the files — `min=0 max={TIME_SAVED_MAX_HOURS}
step="any" inputMode="decimal"`, held in state as a STRING so a half-typed
"6." never becomes NaN, parsed client-side by the same module before send,
and put in the FormData as `timeSavedHours` only when non-empty.
**`step="any"`, never a grid**, and this is a correctness rule rather than a
preference: `<SubmissionForm>` is a real `<form onSubmit>` with a
`type="submit"` button and no `noValidate`, so the browser runs constraint
validation BEFORE the handler ever runs. Under the round's first `step=0.25`
a submitter who typed **6.3**, filled in the rest of the form and attached a
package was refused by the browser ("the two nearest valid values are 6.25
and 6.5") for a value `parseTimeSavedHours` accepts happily and the inline
row editors — which sit in no form and so run no constraint validation —
save without a word. The same field must not refuse in one write moment what
it accepts in the other; the parser is the single arbiter, and `min`/`max`
stay only because they agree with it exactly and give instant native
feedback. The client-side parse is belt and braces and is documented as
such: it does NOT spare the upload (`req.formData()` has already buffered
the package by the time the value exists) and it is close to unreachable in
a browser (`<input type="number">` sanitizes unparseable text to `""`, and
native `min`/`max` already refuse out-of-range). What it does buy is the one
thing worth keeping: the sentence a person reads on a refusal is the route's
own sentence, so there are never two refusal texts to drift apart. Its help copy is
lane-NEUTRAL ("your published card", "the scorecard") because the same
component renders inside the company dialog where "the Our Work page" would
be false. (2) Afterwards, on any of the viewer's own rows at any status:
`/work/submit` rows and the /roadmap/work "In Review" list (§5.18) each
carry an inline editor that POSTs `{hours}` to
`/api/work/submissions/[id]/time-saved`, writes the returned
`timeSavedMinutes` into local state AND re-runs the surface's own refresh
(the 10 s poll on /work/submit, `router.refresh()` on the force-dynamic
roadmap page) so an in-flight response cannot repaint the old number. On
/work/submit that guarantee needed a second half, added the same day:
`refresh()` already dropped replies from a scope the reader had left
(`viewRef`) but had no ORDERING guard, so a poll issued BEFORE a save could
land after it and put the pre-save number back under the confirmation saying
it had saved — and when that late reply also carried the row's terminal
status, `anyActive` went false, the 10 s interval was torn down, and nothing
ever came to correct it. A monotonic `refreshSeq` ref is bumped on every
issue and captured locally, and ONE predicate (`s === viewRef.current && seq
=== refreshSeq.current`) drops a reply that is stale either way. The
existing `setLoading(false)` rule is preserved deliberately: only the NEWEST
request clears the spinner, so a refused or thrown request still clears it
(the newest request always reaches that line, a rejected fetch falling
through the empty `catch`) while a stale reply cannot clear it out from
under a request still in flight.
Refusals render per ROW, never in a panel-level slot (this section's
standing rule: a page-level message is invisible to someone acting on the
last row), and the editor stays OPEN on a refusal with the value still
typed. There is no confirm step: the convention guards acts the presser
cannot undo alone, and this is a retypeable number. Each toggle carries an
sr-only suffix naming the row (`for {title}` here, `time saved for {title}`
on the roadmap island): the visible label is short because it sits beside
the row it belongs to, the announced one is not, and a reader with five
submissions otherwise meets five buttons with one accessible name in a
screen reader's button list — the `CountCell` precedent from the scorecard.
Help copy is conditioned on publication ("Once the card is published the
figure shows on it, reported by you, and counts on the scorecard"): the
editor renders on received, running, held and failed rows too, where there
is no card at all and the published-only scorecard counts nothing, and the
first wording asserted both outright.
The control is offered ONLY on rows the viewer owns — the route would accept
a `verifiedWebAdmin` edit and the all-submissions view shows the value
read-only, but someone else's self-reported estimate is not an admin's to
retype — and, since the same-day refutation, NOT on a `superseded` row
either, whoever owns it. A superseded row is kept visible on purpose (when
the live version belongs to someone else it is the submitter's only surface,
and the roadmap "In Review" list is not status-filtered at all), `isMine()`
is true for it, and the editor used to answer a save there with "Saved. This
submission reports 6 hours a month." while `publishedCards()` and the
scorecard sum both key on `status = 'published'` and read the value NOWHERE
— the same predicate that stops a card and the generation it replaced from
being counted twice. So a superseded row renders the figure read-only when
it has one ("as reported on this version", plus a faint line reading
"Nothing reads this figure now. Whatever the live version reports is what
the card and the scorecard show.") and nothing at all when it does not, with
the existing "Submit an update" button beside it as the honest next step.
That second sentence is deliberately CONDITIONAL and `work-tests.ts` pins it
so: on /work/submit a superseded row survives the dedupe mainly when the live
version belongs to SOMEONE ELSE, which is exactly the case where
`publishWithSupersede`'s same-person guard publishes the child with NO
figure, so the earlier wording ("the live version carries its own") asserted
a number that is not there while /work shows no such line and the reporter's
scorecard total has just fallen. On
/work/submit the superseded test runs FIRST, above `isMine(r)`, because the
admin all-submissions view resolves no `currentId` and never runs the
dedupe, so a superseded row arrives there with `isMine` false and would
otherwise land in the read-only arm by accident rather than by rule; in
`<TimeSavedEditor>` it is an early `return` placed BELOW every hook, so the
hook count cannot differ between a superseded row and its neighbours. Held
and failed rows deliberately KEEP their editor: a retry can still publish
them, and the figure becomes real the moment it does.

Display: `src/components/work-card.tsx` is the ONE card template behind
public /work and the company lane, and it renders `Time saved · {phrase},
reported by the submitter` in a `mono ... text-faint` line directly ABOVE
the footer byline (so the byline stays the card's last line), only when a
value exists. The attribution is load-bearing, not decoration: everything
else on that card came out of the panel, while this number is typed by the
submitter and no review stage ever checks it. /work's section intro carried
the matching promise — "Every claim below is drawn from the submitted
documents" — and it was narrowed in the same round rather than left to be
quietly false: it now ends "..., apart from a time saved figure, which is
reported by the submitter and labelled that way on the card." A comma
clause, not an em dash (owner ban on visible copy), and the sentence names
the exception rather than softening the promise, because the promise is the
reason the card line says "reported by the submitter" at all.

**Update lane: inheritance happens at SWAP time, in one place, and only
between the same person's rows.** The field is NOT rendered in update mode,
the FormData key is omitted, `POST /api/work/submissions/[id]/update` passes
NO time-saved value into `createSubmission`, and an update row therefore
arrives with `time_saved_minutes` NULL on purpose. The inheritance lives in
`publishWithSupersede`, in the child's `.set({...})` that flips it to
`published`, beside `slug` / `publishedAt` / `displayRank` which are already
taken from the LOCKED parent for exactly this class of reason:
...
Inheritance is needed at all because an approved swap makes the child the
published row, so a null child would silently erase the figure from the live
card, and "empty means clear" would be ambiguous on the one form where the
submitter never saw the field. Every clause of WHERE and HOW is load-bearing:

- **At swap time, not at intake.** The round's first shape copied
  `row.timeSavedMinutes` into the update route's `createSubmission` call.
  The time-saved route is deliberately status-blind, so the owner can
  correct the figure on the LIVE parent for the whole time an update waits
  for approval; an intake snapshot is read weeks before the swap and
  silently reverts that correction. Here the parent is read under `FOR
  UPDATE` in the same transaction, so what publishes is what the live card
  was showing a moment ago.
- **One primitive covers every lane.** Both swap paths reach this function
  (the admin approve route calls it; the web auto-approve lane arrives
  through `finishUpdateRow`) and so does the EMAIL update lane, through the
  panel. `email-intake.ts` never passed a value at intake at all, so under
  the intake copy an emailed update republished the card with the figure
  GONE. Fixing the one primitive fixed the email lane without touching it.
- **Same person only.** The number is self-reported and the §5.18 scorecard
  sums it PER PERSON, and an update may be submitted by someone else
  entirely: admins can update anyone's card, and so can any earlier
  participant in the supersede chain (`canProposeUpdate` is CHAIN ownership).
  Carrying Alice's figure onto Adam's row would print a claim Adam never
  made, credit it to Adam's scorecard column under a disclosure paragraph
  saying each person reported their own, and leave Alice unable to take it
  back — the row is not hers any more, so her POST to the time-saved route
  404s. `sameEmail` comes from the PURE `./transfer` (its only imports are
  `@/lib/rfp/access` and `./config`), so the value import into `db.ts` does
  not cycle the way a `./scope` value import would.
- **The child's own value wins when it has one.** Because the route is
  status-blind, an updater can report their own figure on the child while it
  waits, and that is a deliberate statement by the row's owner, not a
  leftover.
- **The known edge, stated honestly.** NULL cannot distinguish "never
  reported on the child" from "cleared on the child", so clearing the CHILD
  while the parent still carries a figure re-inherits it at the swap. The
  gesture that always works is clearing the row that is actually live (the
  parent before the swap, or the published card after it), which is also the
  row a person is looking at when they decide the number is wrong.

`rollbackSwappedUpdate` deliberately does NOT participate: it DELETEs the
child outright and its `.set()` never mentions the column, so the restored
parent keeps the figure it always had and no inherited copy can survive a
rollback and go stale. The scorecard column that sums these values, and the
roadmap-side editor, are specified in §5.18.
...
then live ledger rows newest-first (title, file name, size, store date
(when the FILE entered the store, not the submission's `created_at`),
`#sub-<id>` link while the submission row still exists —
...
projections, no DB/brain; since 2026-08-27 also the whole `time-saved.ts`
contract — both spellings of not-reported, 0 as a CLEAR, trimming, the
1-minute clamp, by-type refusal of booleans/arrays/objects, the 744 cap and
the copy that names it, the `hoursFieldValue` round trip pinned to two
decimals, and the arbitrary decimals a `step="any"` input can now send
(6.3, 0.1, a long tail rounding to whole minutes) — plus three source
tripwires the compiler cannot hold: migration 0049's CHECK bounds equal
`TIME_SAVED_MAX_MINUTES` and the parser's floor, the scorecard sum still
rides the published-only where clause and keeps its `::int`, and the
published card still says "reported by the submitter"; and a SECOND scrape
block for the round-2 fixes, which are all shape rather than value and so
invisible to both types and unit tests: `publishWithSupersede` still carries
`child.timeSavedMinutes ??` and the `sameEmail(parent.submitterEmail,
child.submitterEmail)` guard and reads `parent.timeSavedMinutes`, `db.ts`
still imports `sameEmail` from the pure `./transfer`, the update route
contains the string `timeSavedMinutes` NOWHERE while still naming
`publishWithSupersede`, `rollbackSwappedUpdate` contains it nowhere either,
all three inputs are `step="any"` with no `step={0.25}`/`step="0.25"` and
keep `TIME_SAVED_MAX_HOURS` as their max, both surfaces carry the superseded
read-only copy with /work/submit testing `superseded` BEFORE `isMine(r)` and
the island's early return sitting below every hook in its body, both
editors' copy is conditioned on publication, `refreshSeq` exists with the
combined staleness predicate and the newest-only `setLoading(false)`, both
toggles carry their sr-only row title, `/roadmap/work`'s page passes
`id`/`title`/`status`/`minutes` into the island, and /work's intro carries
the narrowed promise with no em dash in it) and
`npm run test:workupdate` (the update state
...
- **Web intake:** `POST /api/work/submissions/[id]/update` (multipart, same
  ...
  Non-empty `title`/`kind` form fields 400 (never silently ignored). A
  `timeSavedHours` field is IGNORED here rather than rejected, the form does
  not render it, and this route passes NO time-saved value into
  `createSubmission` at all: the child inherits `time_saved_minutes` from
  the parent at SWAP time, inside `publishWithSupersede`, and only between
  the same person's rows (see the update-lane block in the time-saved
  passage above for why an intake copy was wrong three ways). Entry:
...
- **The island.** The toggle sits where the "Your submissions" heading was,
  ...
  hides rows is the same failure as a total that lies). The submitted-at chip
  added 2026-08-25 is the one thing added here that does not vary by view: it
  renders on every row of both lists, because the owner directive says
  "always". On rows the viewer does not own, **Retry review** and **Submit an
  update** are suppressed by render only: retry burns one of that row's three daily panel runs, and an
...
- **Bulk transfer script (`npm run work:transfer`, 2026-08-29).**
  `scripts/work-transfer.ts` is the scripted twin of the POST route above,
  built for the canvas round in which two dozen CoWork skills submitted by
  adam@xl.net on colleagues' behalf had to move to the people the Slack
  canvas credits. It takes a plan file (a JSON array of `{id, to, note}`;
  unknown keys, malformed uuids and duplicate ids are refused loudly, since
  a plan is typed by hand and a silently ignored key would move nothing while
  looking complete), and per row does a FRESH `submissionById` read and runs
  `decideTransfer` (`scripts/lib/work-transfer-ops.ts`, pure and unit-tested
  in `scripts/work-tests.ts`), which calls the route's own `transferTarget`
  and `transferBlockedReason` against the staff lane (`WORK_SUBMIT_DOMAINS`,
  "the Our Work page"). Nothing is re-derived: if the route's policy moves,
  the script moves with it. `--apply` calls the same `transferSubmission`
  compare-and-swap pinned on the owner, status and `panel_attempt_id` that
  read saw, `user_id` via `userIdForEmail` (NULL for a recipient with no
  site account, as the route allows), prints the route's exact
  `[work] transferred sub=… from=… to=… by=…` line with `by=` the mandatory
  `--actor` (an `emailDomain`-parsed xl.net address; a batch with no actor
  would put `by=undefined` in the audit trail), and reports a `raced` row
  and carries on. Dry run is the default and prints the decision table; a
  re-run after a partial apply shows moved rows as `skip: already owns it`,
  not as refusals. **Refused per row, disclosed and never written, exit
  still 0:** a missing row; a company-lane row (`company_id` set: its lane is
  the company's own domain and needs `companyById` plus the paused/eligible
  checks, which is the web control's job); a target the staff lane rejects;
  `superseded` rows and rows in a live panel run (stale heartbeats are
  movable). Exit 1 when a planned move raced or threw, and on a bad plan or
  bad arguments; an aborted confirm prompt exits 0 (`work:import`
  precedent). It writes what
  the route writes and nothing else: never `submitter_name`, `card_json`,
  `slug`, `display_rank` or `published_at`. **Email is new-owner-only and
  opt-in** (`--notify`, meaningful only with `--apply`; refused without it):
  `notifyTransfer` also mails the previous owner and, for published rows, the
  owner mailbox, and for a batch out of one mailbox that is two dozen "your
  submission was moved" mails plus two dozen owner copies into an inbox that
  already receives the OUTBOUND_BCC copy of every send, addressed to the
  person whose attribution the plan is correcting. To make that possible
  without forking the copy, `notify.ts` now exports
  `notifyTransferNewOwner({row, actorEmail})`, the new-owner send extracted
  byte-for-byte (subject, text and the skip-when-actor guard) and called
  internally by `notifyTransfer`, whose behaviour on the route is unchanged.
  Without `--notify` the summary says no email was sent. Runs on the VM as
  the deploy user (refuses root, the `work:reclassify` reason).
...
enforced. PDF turned out not to need Chromium: `pdfkit` renders the
response (the package is in `serverExternalPackages` — it reads font
metrics from its own package dir via fs, and bundling breaks that path);
since 2026-08-27 both emitters embed the vendored real faces and mirror
the on-screen sheet anatomy (§5.17.5) instead of the built-in
Helvetica/Georgia approximations this round shipped with.

**Violation messages carry INSTANTS, not formatted days (2026-08-26).** The
last live member of the viewer-zone class (§4) was inside the gate itself:
rule C1's message interpolated
`correctedAt.toISOString().slice(0, 10)` and the same call on `draftedAt`,
and that string is READER-FACING — C1 is registered in `validators/gate.ts`,
the `GateResult` crosses `POST /api/rfp/proposals/[id]/gate` into
`workspace.tsx` (`setGateResult`, plus a server-prop seed parsed from the
stored `gate_json`), and the Checks pane painted `{v.message}` verbatim. So a
Chicago staffer correcting a fact at 21:30 on Jul 26 read
`Corrected Jul 26, 2026, 09:30 PM CDT` on `/rfp/knowledge` and
`corrected on 2026-07-27` in the Checks pane of the same browser: one
instant, two consoles, two days. C1's entire payload is the ORDERING of
those two days ("corrected on X, AFTER drafted on Y"), so a shifted day is
not cosmetic, it renders the rule's own claim false.
A plain string cannot hold a React element, so the sentence is now carried
SPLIT. `Violation` gains an OPTIONAL `timedMessage: TimedMessage`
(`content-model/gate.ts`), the N-instant generalisation of the
`CheckLine {before, iso, after}` precedent in `lib/roadmap/platform-copy.ts`:
`{ segments: {before, iso}[], after }`, each segment being the prose that
PRECEDES its instant. `message` stays mandatory and complete, and is DERIVED
from the split form by `flattenTimedMessage()` rather than written beside it,
so the two cannot drift; the flat form renders each instant as an explicitly
labelled `YYYY-MM-DD (UTC)`, because it is the degraded path and an
unlabelled day implies the reader's own day. The Checks pane renders
`timedMessage` through `<LocalTime withTime>` and falls back to `message`.
`<LocalTime>` and NOT `exact()`: that pane's `gateResult` can arrive as a
server prop, so these spans are server-rendered on first paint and a
runtime-zone formatter would resolve to the VM's UTC on the server and the
reader's zone in the browser (the hydration-discard class §5.18 records).
Three facts made this cheap enough to do properly rather than papering the
string with a "UTC" label: **C1 is the ONLY rule of the 26 that puts a date
in a message** (the sole other `toISOString` under `content-model/` is
`hash.ts`, which is not a message), `v.message` reaches JSX at exactly ONE
site (`workspace.tsx`; `formatGateResult()` has no caller at all), and the
field is additive on a JSON blob in a `text` column, so it is **NOT a
migration** — a `gate_json` row stored before this change simply lacks the
key and renders `message`, and re-running the gate replaces it. A rule that
ever names a stored instant sets `timedMessage` and lets `violation()`
derive the message; a bare `toISOString().slice(0, 10)` in a rule message is
the defect, and `rule.ts`'s doc comment says so at the call site.
...
with the no-currency prohibition standing regardless of source; POST with
the reserved label `__doc` (`DOC_LABEL`, letter.ts, unforgeable like
`__letter`) is instead a PLAN request for the whole-document flow below,
returning `{plan: {targets: [{label, op, directive, heading?}], note}}`
(op ∈ revise|retitle|remove; heading present exactly on retitle) and
logging `proposal.tron_plan` shape-only (+ `retitles`/`removes` counts);
the per-section POST also takes an optional `directive` (JSON or form
field, 600-char cap), the planner's per-section output round-tripped
through the client and re-entering the revise prompt only through
`fenced()`, with `directiveChars` in the activity meta, and its response
carries optional `heading`/`remove` structural proposals for real (non-
letter) sections; PATCH with `op: "retitle"|"remove"` applies an ACCEPTED
structural proposal — see the whole-document flow below — logging
`proposal.section_retitle`/`proposal.section_remove` shape-only),
...
**Whole-document Tron revision is PLAN-THEN-EXECUTE (owner ask 2026-08-27),
and it is the DEFAULT scope (owner ask 2026-08-28).**
The Tron pane's scope picker offers "The whole document" alongside the
sections and starts there. A plan turn (`planDocumentRevision`, sessionId `rfpplan_*`, 90s
timeout; the route branches on `DOC_LABEL` BEFORE the per-section 404
lookup) reads every drafted section, including `__letter` under its RAW
label — labels are the targeting keys and a client section can legitimately
be TITLED "Cover Letter", so the revise turn's display-title convention
deliberately does not apply here — and returns which sections must change,
each with an `op` and one directive (500-char cap). **Targets carry ops
(owner ask 2026-08-28: "rename the incumbent" and "remove past-meeting
references" both lived in section HEADERS, which a body-only revise could
never touch, so the same instruction was re-asked for days):** `revise`
(the default) loops through the EXISTING per-section revise call
sequentially, exactly as draft-all drafts; `retitle` carries a
model-authored `heading` (cleanHeading: single line, angle-run strip,
`stripReservedPrefix`, 120 chars) and `remove` needs nothing further —
both render as INSTANT proposal entries in the client with no extra brain
call. One synchronous whole-document rewrite stays impossible under the
100s edge, and parallel calls would starve the shared semaphore. Nothing
writes until a human accepts each proposal: body revisions go through the
same staleness-guarded PATCH as ever, and an accepted retitle/remove goes
through the PATCH's `op` branch, which refuses any reserved "__" label,
re-cleans the heading server-side, and applies ONE transaction
(`writeProposalStructureOp`, CAS-on-rev first) across `sections_json`, the
document's `structure_json`, and the `rfp_requirements` rows that key
Coverage (renamed on a label rename, deleted on a remove). WHICH slot a
retitle fills follows what the reader sees (`labelDisplaysWorded` in
letter.ts, the secKicker/sectionKicker display rule, parity-pinned in
`npm run test:rfpletter`): a worded label ("June 8th, 2026:") IS the
visible header, so the LABEL renames AND the title CLEARS — the heading is
contractually the full replacement header, both slots print (kicker + h3,
mirrored by the export), and a preserved title would keep the very words
the request asked to strip on the sheet and in the delivered file
(refuter catch) — uniqueness-checked against both stores, echoed back as
`{label, title}` for the client to migrate its state; a bare-numbering
label ("4.2", rule C4's client numbering) keeps its label and the heading
lands in the title. A revise proposal that
also carries a `heading` accepts as `op: "retitle"` WITH its paragraphs,
so header and body land in the same CAS'd write. Two hard server guards
(both refuter catches): a remove that would empty the structure is refused
(409 — the workspace renders the whole document THROUGH structure, and an
emptied structure_json has no restore path short of re-ingesting), and the
generate route refuses any non-letter label with no structure node (labels
are mutable/deletable now, and a stale tab's "Draft this" on a retired
label would otherwise resurrect it as a record invisible to every
workspace yet appended to the export by resolve-draft's orphan pass;
residual: a remove accepted during the seconds a draft of that same
section is in flight can still land an orphan — no realistic path, the
plan only targets drafted sections). The single-section revise
turn offers the same two structural outputs when the request asks
(`allowStructural`, false for the letter: its header and existence are
furniture on every path — plan validation forces `__`-labels to revise,
the PATCH refuses them, and the letter's revise turn never sees the
option). Removal receipts render in words in the Tron pane on BOTH accept
paths (`tronRemoved` for the single proposal, `docRemoved` for the
whole-document list — there is no sheet left to flash, the vanished entry
is not confirmation, and the doc-flow "Stopped" line counts removals so it
never claims nothing was written after a delete); the bulk "Use all"
button and the aggregate line both name their removal count. A removed
section's structure node, section record, and requirement rows all leave
together. Client hygiene from the same refuter pass: every scope-changing
route (the per-section "Ask Tron" buttons included) runs the select's full
clears via `pickScope`; an abandoned doc run RETURNS at the loop top
...
the fence cap can never silently hide a tail section from the plan (a plan
that never saw a section cannot select it, and `draftCoverLetter`'s
content-only arithmetic would have allowed exactly that). Targets are
validated select-never-author against the stored labels with a
trim-tolerant match that always returns the STORED label, because the
revise loop matches sections by exact string. Client-side, a run-generation
ref detaches an abandoned loop (scope change mid-run) so a cleared pane
cannot repopulate from in-flight responses, the busy line is keyed on the
run as started rather than the current select value, Use-all freezes the
per-entry buttons for its duration, and the stopped line is phrased off the
accepted-count at render time so "nothing has been written" is only ever
...
state ALWAYS downloads (owner directive, round 4) — UNMARKED (owner ruling
2026-08-28: the file must not say DRAFT anywhere).** Both emitters
...
in full via the engine's own `missing[]`) is not refused AND not marked:
the round-4 in-file marking (cover line, PDF corner mark, DRAFT footer,
`-DRAFT` filename) is REMOVED — `ExportView` no longer carries `draft` —
and what is outstanding travels ONLY in the
`x-rfp-draft`/`x-rfp-open-gaps`/`x-rfp-pricing-missing`/`x-rfp-gate-passed`
response headers, which the workspace turns into its "Downloaded. Still
outstanding: ..." notice. For the record: refusal was the original B7
anti-exfiltration backstop, round 4 replaced it with in-file marking, and
this round removes the marking on explicit owner ruling — the residual
...
the DOM for the mobile Draft tab), rendered inside `.rfp-docpane`.
SINCE 2026-08-28 the docpane is NOT sticky and NOT self-scrolling at any
width (owner: the stacked sticky bars plus per-column scrollbars made the
workspace hard to maneuver): the WINDOW scrolls the document, `jumpTo`
always uses `scrollIntoView` with the `sec-*` scroll-margins clearing the
sticky bars, only the RAIL still self-scrolls, and the change receipt
(`.rfp-doc-receipt`) is sticky against the window below the header stack. The document itself is `.rfpdoc`: the Proposal Studio handoff's
...
render this same family since 2026-08-27 (§5.17.5 — until then they
emitted a plainer flowing style, deferred on the record, not silent). Gap questions DEDUPE by
...
benchmark tool's four or five). Since 2026-08-27 (the B-O-F
repeat-question incident: one co-managed-IT question minted under four
wordings across eight sections, answered three times over) the "same
question" vocabulary lives in ONE pure client-safe module
`src/lib/rfp/gaps.ts`: `normalizeGapQuestion` (byte-identical to the old
workspace inline key, treated as a persisted format), `collectOpenQuestions`
(other sections first in document order, the target's own current gaps
last, "__" labels never contribute, first normalized occurrence wins),
`capOpenQuestionsForPrompt` (12 questions / 6000 chars, never truncating an
individual question, prompt list only, with a reserveTail that admits the
redrafted section's own trailing gaps before the head fills the cap, so a
busy proposal cannot evict exactly the wording whose stability keeps a
merged entry from splitting) and `snapGapQuestions`. The generate
route passes the proposal's already-open questions (claim-time snapshot,
capped) into `draftSection` as a REQUIRED parameter; the prompt gains two
conditional blocks (byte-absent when nothing is open) forbidding a gap that
asks for information a listed question already asks for and requiring a
character-for-character copy of the listed question when the section
genuinely needs it, so exact copies merge and one answer weaves everywhere.
A deterministic landing-time snap inside the CAS retry loop then rewrites
any returned gap that normalizes equal to a currently-open question
(collected UNCAPPED and fresh, so questions answered mid-draft are not
re-minted under stale wording) onto that question's exact raw text. Open
questions ride the prompt under the requirements discipline (single-line
collapse plus angle-run strip, operator voice, never fenced); the collapse
is safe ONLY because the snap folds a collapsed echo back onto the stored
raw, which requires both transforms to map affected runs to a SPACE, never
the empty string (an empty-string strip fuses neighbors and breaks
normalize-equality; refuter-caught, test-pinned). The letter keeps `gaps: []` and receives none of this. Pre-existing
paraphrase variants in stored data are not migrated; they merge as their
sections are redrafted or get answered individually. Tests:
`npm run test:rfpgaps` (scripts/rfp-gaps-tests.ts, pure).
...
part dividers included, since 2026-08-27 (§5.17.5).
...
hyperlinks (docx `ExternalHyperlink`, pdfkit `link:`); since 2026-08-27
the block is set the way the screen's `.rfpdoc-sig` sets it (serif runs
in the signature palette) rather than Tahoma approximated by Helvetica
(§5.17.5).

#### 5.17.5 Export fidelity: the downloads ARE the workspace document (2026-08-27)

Owner directive: the downloaded Word and PDF must look exactly like the
sheets on screen, and until this round they did not (the emitters still
spoke the plainer round-3 style while the pane had moved through rounds
5-8 of CoWork anatomy). Both emitters in `src/lib/rfp/export.ts` are now
recomposed page-per-sheet, mirroring the workspace sequence one-for-one:
arc-mark cover (vector circles in the PDF, replicating the CSS
`::before`/`::after` geometry proportionally — the on-screen sheet is
17/22, exactly US Letter, so screen px map to pt by one scale factor),
cover-letter sheet with the pagehead logo + kicker over the navy rule,
the two numbered part dividers with ghost numerals (stroked text in the
PDF: stroke `#2f31c5`, fill `#eef0fb`), one sheet per section with the
SAME kicker logic (`sectionKicker` in the new
`src/lib/rfp/export-assets.ts` is a character-identical replica of the
workspace's `secKicker`, parity table-tested across 21 label shapes),
the Investment sheet's branded table (navy thead with white letterspaced
caps, zebra `#f9fafd`, semibold first column, ink-ruled total rows), the
solid-navy closing sheet with the white wordmark, and the pagefoot
(hairline, "XL.net · Managed IT Services Proposal" / "Confidential") on
every page of both formats. Furniture strings are single-sourced in
export-assets.ts and match the JSX verbatim; every money figure prints
through `formatMoney` with cents always, so `$247.00` reads exactly as
the screen's `fmtCents` renders it, and a figure never line-wraps (the
PDF steps the font down when a total outgrows its column).

REAL FONTS in both formats. `public/brand/fonts/` vendors the OFL static
TTFs of the faces the screen loads from Google Fonts — Archivo
...
obfuscated embeds (verified against LO 25.2), and plain font parts are
the form LibreOffice's own DOCX exporter writes, which Word also reads;
worst case a Word reader installs the four faces and the run-level
family names are the real ones. Word-format approximations, on the
record: the cover arc and the divider ghost numerals are 300dpi
pre-rendered PNGs in `public/brand/rfp-ornaments/` (OOXML has no
text-stroke), the navy closing sheet is a fixed-height shaded table, and
cover/divider vertical rhythm uses fixed spacers rather than true
bottom-pinning. `loadRfpExportAssets()` fs-reads fonts/logos/ornaments
from `process.cwd()/public/brand` with a module cache (a missing asset
throws ENOENT cleanly — route 500 — and a transient failure does not
poison the cache). THE ASSETS ARE A RUNTIME DEPENDENCY THE BUILD DOES
NOT CHECK: `tsc` and `next build` pass without them, so they must ship
in the same commit as the code (they live under `public/`, which the
deploy rsyncs).

`ExportView` gains `contactPhone`, `sections[].kicker` and
`closing {headline, lede}`; `buildExportView` stays pure and every
exported signature is unchanged, so the export route's contract
(x-rfp-* headers, always-download draft marking, the buffered-pass
...
counterpart refuter (owner-directed pairing); refuter verdict
SHIP-WITH-MINORS, all four findings fixed in the screen-matching
direction (cents-always money, the DOCX footer's extra page number
dropped so it matches the screen's "Confidential", empty-client lede
guards, the money wrap guard); its hostile-input sweep (XML metachars,
...
behavior is identical to expiry. The approver screen's "Requested X ·
expires Y" line renders BOTH halves through `<LocalTime>` since 2026-08-25:
it is a server component, so both used to resolve to the VM's
UTC, and because `expiresAt` is `createdAt + 7 days` they carried the
identical wrong hour-of-day — a request filed 01:30 UTC read "Aug 25 ...
Sep 1" to a Chicago approver whose civil dates were Aug 24 and Aug 31. They
move together by rule; converting one half alone leaves the sentence
disagreeing with itself. Both took `withTime` on 2026-08-26, OVERRULING the
in-file argument that "a clock on one half of a 7-day window reads broken
beside a bare date on the other, and no approver acts on the minute": the
owner's ruling puts a clock on deadlines specifically, and the requester's
mirror of this same row (`components/roadmap/request-admin.tsx`, "Admin
access requested X ... expires Y" on `/roadmap`) moved in the same round, so
leaving this half date-only would have made one deadline read two ways on
two pages. That requester-side island is worth knowing as a shape: it is
`"use client"`, but `/roadmap` resolves `pending` server-side and the
component's first statement returns the date branch, so it is SSR'd — it
was never a live mismatch only because the old `fmtDate` pinned UTC. Suspend/rename/grant/revoke/purge live in
...
Admins/Created by/Created. Every stored date this console shows renders in
the VIEWER's zone through `<LocalTime>` since 2026-08-25 (they were bare
`toLocaleDateString`, i.e. the VM's UTC wearing no label), and since
2026-08-26 every one of them carries a CLOCK. The companies table's
"Created" cell and the governance-doc "added by X, DATE" parenthetical in
BOTH detail branches took `withTime` under the owner's "clock on everything"
ruling, closing what this section had recorded as an outstanding exception
rather than a settled decision. So did the pending admin-access row in
`actions-client.tsx`, which had stayed date-only for a LAYOUT reason and not
a zone one: it is a compact flex line and a clock's post-mount widening
wrapped Approve/Deny onto a second row. The answer was placement, not
precision — the timestamp now sits LAST in that row, after Deny, because
flex-wrap only ever displaces what FOLLOWS the item that grew (the §5.16
`/work/submit` rule), so the buttons hold fixed offsets and the timestamp
wraps alone; the identity clause lost its trailing comma with the move. The
last deliberate date-only surface anywhere, `when()`'s >7-day branch on
`<When>` lanes, closed in the SAME pass (its `ABS` formatter was folded into
a clocked `ABS_TIME`). That leaves the class with exactly ONE standing
exception, and it is named here so this paragraph cannot be read as a
blanket closure: `/admin/work#storage`'s `dateLabel` (§5.16,
`src/app/admin/work/page.tsx`) renders `work_archive_files.created_at` in
UTC on purpose, with a baked `" UTC"` suffix, because it is the archive
FILE's store date and `work:backfill`/`work:import` stamped those rows long
after the submissions they hold. It is a DIFFERENT FACT from a submitted-at,
not the same fact in the wrong zone, which is why the viewer's zone is not
the right answer for it. Nothing else is excluded. §4's `local-time.tsx`
bullet carries the mechanism, and the triage rule at the end of it carries
the burden-of-proof wording that keeps this carve-out settled. Meanwhile
the work_submissions "Created" column takes `withTime` in BOTH detail
branches, matching the labelled clock §5.16 `/admin/work` puts on the same
fact — the two consoles are required to agree. The synthetic staff row's "·"
placeholder is untouched by all of this: that lane has no `createdAt`. The
pending-request queue inside `actions-client.tsx` also moved, and that one
was a live DEFECT rather than a wrong zone: the island is `"use client"` but
is statically imported and rendered by the server page (no `next/dynamic`,
no `ssr: false` anywhere under `src/app/admin`), so the App Router SSRs it
and the bare formatter ran twice, UTC on the VM and the reader's zone in the
browser. For any request whose UTC date and the reader's civil date straddle
midnight (00:00-05:00Z for a Chicago reader, 22:00-24:00Z for Berlin) the
two strings differed, which is a text hydration mismatch; with no Suspense
boundary between the island and the router root, React discarded the server
HTML for the WHOLE page and client-rendered it again. `exact()` cannot fix
that class (it formats in the runtime zone on the first render, which during
SSR is still the VM) — only `<LocalTime>`'s UTC-pinned seed can, which is the
general rule recorded in §4's `local-time.tsx` bullet. That synthetic staff
row links to `?companyId=staff`, a literal
...
hard-locked. RUNWAY RULES FOR PAID STEPS: `TRACKED_STEP_KEYS` (the nine
...
Promise.all). Availability-neutral, and still never task progress — but the
NODE now shows it (owner override 2026-08-20, superseding the same-day
"informational only, node semantics unchanged" ruling for the node visual
ONLY): when the lane's count for a paid step is > 0 the runway layers
`.rmp-node--attended` over `--offered` (a MODIFIER in the `--partial`
tradition, second arg of `nodeClass`; computed only for the two untracked
keys so it is offered-only structurally). The class keeps `border-style:
dashed` (the paid identity) and borrows the up-next brightness recipe —
`border-color: var(--xl-light)`, 2px weight, `0 0 8px var(--xl-light-glow)`
— with no animation (pulse means working only); it is declared after
`--offered` because both are (0,1,0) and only source order lets it win. The
sr/tooltip phrase layers too: "Booked separately, {n} attended". Segments,
`UPNEXT`/frontier, `reachedToward`, and the progress percentage still ignore
attendance entirely. Wiring: `RunwayStatus` grew an OPTIONAL `attendance`
field, so all four authenticated runway call sites (company hub, staff hub,
both (steps)-shell arms) carry it automatically by passing their existing
full status bundles; the signed-out teaser passes `status={null}` and never
renders it.
...
from "second row" to "refresh" as well; EDIT-AGAIN (owner directive
2026-08-20: "Even final governance should be editable in the future"):
`POST /api/roadmap/docs/[id]/edit` brings a Builder-snapshot row back into
the builder FOREVER - within the source project's 30-day life the builder's
own reopen already covers edits, so this route closes the after-retention
gap where the snapshot was the only surviving copy of a company's final
document. Same attach-lane gate (docs-gate "attach"); the doc id comes from
the URL and the lane ALWAYS from the session, bound into ONE strict-uuid
lane-scoped query (`governanceDocForEdit` in roadmap/db.ts;
missing/not-owned/wrong-source/no-text share the download route's identical
404; a NULL or unknown `governance_kind` answers that SAME 404 - never a
fallback kind, which would parse the snapshot against the wrong blueprint
allowlist). If the caller still OWNS a live project with the row's
`governance_project_id` the route answers `{projectId, created:false}` and
writes nothing (the workspace offers Reopen there); otherwise it parses
doc_text back into GovernanceDoc[] (`parseSnapshotMarkdown` in NEW
governance/snapshot.ts, the exact inverse of `projectMarkdown`, which moved
there from the docs route so the pair cannot drift: lenient, never throws,
empty yields zero docs and a 409; doc slugs and section ids are REBUILT
against the kind's blueprint because turn ops validate against
`docSlugAllowlist` - an off-allowlist slug would seed a project that LOOKS
editable while every revise op silently dropped - and overflow docs FOLD
into the last real doc rather than discarding text; STUB STATUS RESTORES
for docs whose shape is provably the stub shape - blueprint-default stub
OR set_stub-evidence-bearing, with every other section a byte-exact
blueprint placeholder - because a confirmed FINAL legitimately carries
stub docs whose scaffold placeholders the confirm gate skipped, and
without the flag they would resurface placeholder-blocked on stub:false
docs and 409 the re-confirm of a document that WAS final, while anything
looser would launder undrafted scaffold past the gate on a non-final
attach. A determination section counts as set_stub evidence only when the
blueprint does not itself ship one or its text differs byte-wise from that
shipped placeholder: two NON-stub eu_ai_act docs (aia-applicability-memo,
aia-prohibited-screening) scaffold a "Determination and adoption" section,
and reading the untouched scaffold as evidence would flip them to stub on
a pure scaffold round-trip. Accepted residual, conservative by design: a
doc that was partly drafted and THEN set_stub'd (determination + drafted
sections + remaining placeholders) re-imports stub:false and re-confirm
409s until a revise/set_stub turn restores it - anti-laundering wins;
sections
beyond maxSectionsPerDoc fold into the last kept section and a body over
sectionMarkdownMaxChars splits into continuation sections, both
byte-preserving), seeds a NEW
caller-owned project directly in status `review` (`createImportedProject`
in governance/db.ts: the parsed documents, a ONE-row transcript with
numberless `qId:"imported"` ("Brought back for editing", the reopen-row
pattern in question-pane), host `REVIEW_IMPORTED_SUMMARY` wrapped in
`withOpenItemsNote` over the parsed docs' `openConfirmTotal` (a snapshot
CAN carry [TO CONFIRM] markers - the manual attach lane has no marker gate
- and a review summary must never read ready-for-final over them),
changed_sections "{}", covered "[]", answersCount 0, research_json NULL -
turn-runner/prompt/view all tolerate a null brief and review-status paths
never force-flip), then RE-KEYS the doc row's `governance_project_id` to
the new project (`repointGovernanceDocProject`, lane + source riding the
WHERE) so the new project's confirm-final auto-attach REFRESHES this exact
row: last editor wins the refresh key; a colleague's still-live original
project inserts a fresh row on its next confirm (accepted, mirrors the
no-unique-constraint acceptance). Gate order: roadmap write kill switch,
attach lane, `governanceEnabled` 503 (a seeded project in a paused
workbench is a trap - the reopen route's reasoning), per-minute
`roadmap:docedit:` throttle (60 s / 10), then ALL validation - row fetch,
parse, the builder's active-projects cap and per-person creates/day budget
with the ADMIN_EMAIL exemption (the seed is zero AI calls but IS a project
row; refusals reuse the create route's copy and fire `notifyBudgetHit`),
lane-derived domain (`normalizeDomain(lane.internalDomain)`), documents
byte cap - BEFORE the fixed-hour `roadmap:docs:` token is spent (the
2026-08-09 lockout mechanic). Responses: 200 `{projectId, created:false}` /
201 `{projectId, created:true}`. UI: an "Edit in the Governance Builder"
island on every Builder-source on-file row (company lane every member,
staff lane global admins only - the page's `canEdit` mirrors the server
gate) that router.pushes `/governance/<id>`, plus one faint promise line
("...even after the original project has expired"); pinned in
governance-tests block 33 (round-trip, leniency, copy, question-pane
tolerance) and the roadmap suite's edit-again section; admins may also LINK
...
+ company cards via the shared card template; since 2026-08-27 every row in
the viewer's OWN "In Review" list carries the `<TimeSavedEditor>` island
(work-islands.tsx, beside `<RetrySubmission>`) on every status EXCEPT
superseded, published included, hitting the same `POST
/api/work/submissions/[id]/time-saved` as /work/submit and calling
`router.refresh()` afterwards because this page is force-dynamic and prints
the same figure on the published card below; its `override` state starts
UNDEFINED rather than seeded from the prop, so a refresh-driven prop change
is not frozen out by a stale `useState(minutes)`; the page passes `id`,
`title`, `status` and `minutes`, the last two both added by the same-day
refutation - `title` only ever reaches the toggle's sr-only suffix (five
rows otherwise mean five buttons named "Edit" in a screen reader's button
list, the `CountCell` precedent), and `status` drives ONE decision, an early
`return` placed BELOW every hook in the component: a superseded row gets no
editor, because nothing anywhere reads its figure (the company card list and
the scorecard sum both select `status = 'published'`, the same predicate that
keeps a card and the generation it replaced from counting twice), so an Add
control there would take a number, confirm it saved, and send it nowhere.
That branch is DEFENSIVE on this page rather than live: `mySubmissions(p.email)`
is not status-filtered, but a COMPANY row can never reach `superseded` at all
(migration 0035's `work_sub_company_no_update_ck` forbids a company row with a
`parent_id`, the update route refuses company parents, and
`publishWithSupersede` re-checks both lanes inside its transaction). It exists
so the shared island stays correct if it is ever pointed at the staff lane -
that row shows the figure read-only when it has one, in company-lane copy
naming "your published card" and "the scorecard" rather than any XL.net-only
page, and renders nothing at all when it does not; the editor's help copy is
conditioned on publication ("Once your card is published ...") because the
island also renders on in-review, held and failed rows, where there is no
card and the published-only scorecard counts nothing; the admin "All
{company} Submissions" list deliberately did NOT get it, that view being
metadata-only and someone else's self-reported estimate not being an admin's
to retype; both submission lists show a
labelled `Submitted` time with a clock in the VIEWER's zone via `<LocalTime>`
since 2026-08-25; these two rows were FIRST, and on 2026-08-26 the ruling
extended to the whole class, so no step page keeps the UTC date-only
`fmtDate()` any more (that helper now has no callers at all), and the admin caption says "titles, status, submission times, and
submitter only"), 05 request (§5.19 form +
...
because the sr-only/link treatment exists to make a link named "2"
identifiable in a screen-reader link list and there is no per-person
time-saved list to click through to; faint at 0 like Published, faint `·`
when the person has no email on file, since an unmatchable person has no
figure rather than a zero one) and its `scorecard/requests`
...
AddToDirectory lever. Its `last` projection carries a DRIZZLE TRAP fixed on
2026-08-26 and worth knowing before writing another aggregate here: the
postgres-js driver installs a transparent (identity) parser for timestamptz
OID 1184 and drizzle decodes on the way out per COLUMN, in
`PgTimestamp.mapFromDriverValue`. A raw ``sql`max(...)` `` field has no
column behind it, so it gets `noopDecoder` and the raw postgres TEXT arrives
untouched. The projection was annotated `sql<Date | null>`, which TypeScript
believed and the runtime did not; it stayed invisible while the value only
ever reached a helper typed `Date | string`, and became a 500 for every scope
with a published card the moment a caller reached for `.toISOString()`. It is
now `sql<string | null>` (matching the honest precedent in
`governance/admin-db.ts`, which types the identical `max()`/`min()`
construct as a string) and `scorecardRows` coerces to a `Date` ONCE, at the
row build, so no consumer has to know the field came from a raw expression.
**Annotate a raw `sql<>` timestamp as a string; only a column select yields a
`Date`.** The 2026-08-27 time-saved column is the same lesson read in the
other direction: ``timeSaved: sql<number>`coalesce(sum(time_saved_minutes),
0)::int` `` rides that SAME select, where clause and group by, and the
explicit `::int` is what makes the annotation true, because `sum()` over an
integer column returns bigint and postgres-js hands bigint back as a STRING
to protect precision - without the cast every consumer that added the field
to a number would concatenate instead (`count(*)::int` on the line above is
the honest precedent, and the `coalesce` is because `sum()` of an all-NULL
group is NULL, not 0, while `ScorecardRow.timeSavedMinutes` promises a
number). Sharing the where clause is not incidental: the sum is
PUBLISHED-ONLY by exactly the predicate that keeps held, failed and
in-review rows off this page "so it can never reveal that a colleague tried
...
the same with "each submitter" and XL.net named - appended LAST so no
existing "It" lost its antecedent.
Downloads (`GET /api/roadmap/docs/[id]`): principal-scoped
...
ROADMAP_BRAIN_DAILY_CAP (1200; raised from 600 on 2026-08-25 so
`admitCompanyRun`'s headroom check against `WORK_CAPS.brainCallsWorstCasePerRun`
= 18 keeps company admission at 60 runs/day), ROADMAP_PANEL_RUNS_DAILY_CAP (60),
...
**Timestamps (2026-08-26).** `components/requests/serialize.ts` no longer
pre-formats dates. It emits RAW ISO instants named with an `...At` suffix
(`requestedAt`, `completedAt`, `createdAt`, `submittedAt`); the scorecard
click-through row's equivalent is `dateIso`, which is NOT built here — that
page (`(steps)/scorecard/requests/page.tsx`) does not import the serializer
and assembles its own row, which is also why it is the one field without the
`...At` suffix. The serializer feeds THREE islands (`my-requests.tsx`,
`pending-queue.tsx`, `request-board.tsx`); a FOURTH,
`(steps)/scorecard/requests/requests-list-client.tsx`, renders `dateIso`,
which the serializer does not build. All four render their instants through
`<LocalTime iso withTime />`; only dollars and labels still cross as strings.
This inverts the previous contract, which sent finished strings from
`fmtDate` precisely so the islands could stay presentational. That contract
was wrong twice over: the helper pinned `timeZone:"UTC"` and dropped the
clock, so a request filed at 20:30 Chicago was labelled the NEXT day on the
board of the person who filed it. Formatting could not simply move into the
islands either — the serializer's three all carry `"use client"` but three
async `force-dynamic` server pages import them STATICALLY, so the App Router
server-renders them with the rows already present, and a runtime-zone
formatter (`exact()`) resolves to the VM's UTC on the server and the reader's
zone in the browser, which is the route-wide hydration discard §5.18
records for `actions-client.tsx`. `<LocalTime>` is the only helper that
crosses that boundary. Two mechanical traps are load-bearing and are the
likely regression sites: in `request-board.tsx` the completed clause was a
TERNARY RETURNING A TEMPLATE LITERAL, and an element cannot live in one, so
it became a fragment that re-supplies the leading space and middot by hand
(`{" · completed "}`) because those characters used to live INSIDE the
literal and the newline between JSX children is a whitespace-only node JSX
strips; the sibling `developerLabel` ternary deliberately keeps its template
literal, since it carries no timestamp and converting it would only be
another chance to lose a separator. Nulls stay explicit: `completedAt` and
the scorecard `dateIso` are `string | null` and the island renders a bare
middot rather than mounting `<LocalTime>` on nothing, which replaces an old
`fmtDate(...) || "·"` that leaned on the helper returning `""`. None of
these fields is ever sorted, compared or used as a key (every list is ordered
in SQL on the real column before serialization), which is what made the
reshape safe.
...
Since 2026-08-26 the five verification sentences (`reachedLine`, `internalLine`,
`attestedLine`, `graceLine`, `failureLine`) no longer return finished strings:
each returns a `CheckLine {before, iso, after}` split at its one timestamp, and
`FieldState` in `components/roadmap/platform-islands.tsx` assembles the sentence
in JSX around `<LocalTime iso withTime />`. A `plainLine(text)` wrapper puts a
dateless sentence (the unchecked state) in the same shape, so `FieldState` needs
no branch; `iso: null` renders the whole sentence from `before` alone.
`checkDate()` is gone. Neither half
of the viewer-zone ruling could be met with a string here: the island carries
`"use client"` but is SEEDED FROM SERVER PROPS by the three async
`force-dynamic` step pages, so these sentences are server-rendered and an
unpinned formatter would resolve to the VM's UTC on the server and the reader's
zone in the browser; and `<LocalTime>` is an element, which cannot be
interpolated into a template literal. `graceLine` was in scope precisely
because "It still counts until X" is a future DEADLINE. `before` and `after`
carry their own spacing and punctuation, so callers concatenate them with
nothing in between, and `iso === null` puts the WHOLE dateless fallback in
`before` so a caller that renders both unconditionally is always correct. The
copy rule that every decided state carries its date is unchanged and still
pinned by `scripts/roadmap-tests.ts`, which now asserts on `line.iso` rather
than on words inside a string. Two consequences worth knowing: a private
`instant()` guard preserves the old helper's invalid-date tolerance, because
`<LocalTime>` has none and would throw out of Intl and take the step page down;
and `FieldState`'s `<p>` is a live region (`role="alert"`/`"status"`), so a
screen reader may re-read the line once when `<LocalTime>` swaps zone a tick
after hydration — accepted deliberately, since lifting the date out of the
sentence would cost the five rungs the distinct wording they exist to have.
...
work_submissions   id uuid PK, user_id uuid NULL FK users ON DELETE SET NULL (published cards
                   ...
                   reviewed document attached beside the package, second retained
                   original, same lifecycle as archive_data, md_data also excluded via
                   ROW_COLS; NULL on legacy rows and on any submission that attached
                   nothing, but NO LONGER Skill-only since 2026-08-28 - a Code program's
                   architecture doc lands here too, which is why notify.ts labels the
                   retention hash line from md_name and not the literal "SKILL.md"),
                   ...
                   time_saved_minutes integer NULL (migration 0049: §5.16 "time saved
                   per month for you" — the submitter's OWN estimate, SELF-REPORTED and
                   never panel-verified, which is why every surface that prints it
                   attributes it. NULL = not reported (the honest state of every
                   pre-existing row and of every email-intake row); entering 0 hours
                   CLEARS back to NULL, so nothing-reported has one storage spelling.
                   Whole MINUTES while every input asks for HOURS, so "6 hours 30
                   minutes" stays exact and no float reaches the column. Migration-only
                   CHECK work_submissions_time_saved_ck (NULL OR 1..44640; 44640 = 744
                   hours = every hour of a 31-day month) — drizzle cannot model CHECKs,
                   so it is invisible to the schema file and `push` would drop it.
                   The update lane INHERITS the parent's value at SWAP time, inside
                   publishWithSupersede, from the parent it has locked FOR UPDATE, and
                   ONLY when sameEmail(parent.submitterEmail, child.submitterEmail) --
                   the child's own value wins whenever it has one. Inheritance at all
                   because an approved swap makes the child the live card; at the SWAP
                   rather than at intake because the time-saved route is status-blind
                   (so a correction on the live parent while the update waits must not
                   be reverted by a weeks-old snapshot), because the email update lane
                   never runs the web route (an emailed update would republish the card
                   with the figure gone), and because an update from a DIFFERENT person
                   would otherwise print one person's self-reported number on another
                   person's row and in that person's scorecard column, unclearable by
...
`scripts/refresh-tron-knowledge.mjs` was DELETED (c0370c8); the module crawler is the only one,
installed by `deploy/setup-vm.sh` as the `aiwebsite-knowledge` timer.

- **Crawl**: 100 % of HTML pages on `https://ai.xl.net` then `https://xl.net`, in that order.
  `knowledge.crawlOrigins` order is LOAD-BEARING: `main()` loops the array sequentially with
  `await crawlSite(...)` and no per-origin catch, so an origin that hangs costs every origin
  after it. xl.net is third-party WordPress this host neither controls nor serves and is the
  only origin that can realistically stall, so it is crawled LAST — before 2026-08-26 it ran
  first, and a hang there meant ai.xl.net was never crawled at all. `knowledge.coreOriginFirst`
  is NOT this: it only ranks pages inside the prompt doc (sink 2) and never affects crawl order.
  Seeds from each
...
**Known consequence of that order, and the thing to expect on the first hit
after any ADD COLUMN:** `next build` prerenders the ISR pages while the new
column does not exist yet, so a page whose query selects it throws during
prerender. On /work that throw is CAUGHT (the `try`/`catch` around
`publishedCards(INTERNAL_SCOPE)`, whose whole point is that the
hand-authored exhibits never depend on the DB), so the build succeeds, the
team-card section renders EMPTY into the build output, and the page serves
that empty snapshot until its first regeneration after cutover: the team
cards simply look gone. It is not this round's
column, it is a property of every column this table has ever gained, and
nothing is actually lost — the cached page is stale, not the data. The fix
at deploy time is one request: hit the page once after `>>> CUTOVER
COMPLETE` and the regeneration picks up the migrated schema. Reversing the
order is NOT the fix: migrating before the build would leave a failed build
with a mutated database, which is the tradeoff this order was chosen for) →
...
wrapper because an in-place guard would trip the drift check" reasoning no
longer applies: the guard lives in `deploy.sh.tpl`, so it renders in with a
matching `aicompany-template:` stamp. It exists because a wrapper only protects
the caller who remembers the wrapper exists. On 2026-08-25 a peer session ran
`deploy/deploy.sh` directly to ship a module pin and rsynced another session's
uncommitted, mid-review round (12 modified + 2 untracked files) to production;
the module-side audit then found `itsupportchicago` and `topmspnearme` had no
tree gate at all, so the protection was absent on half the fleet rather than
merely walk-past-able. The gate prints every offending path, says several
sessions share the checkout, and exits 1 before touching the VM.
