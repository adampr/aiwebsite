...
runs ONE transaction that locks the submission's ledger rows FOR UPDATE,
re-checks `deleted_at IS NULL` and re-stats every expected file at its
recorded size INSIDE the transaction, then NULLs `archive_data`/`md_data`
