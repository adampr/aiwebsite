...
is the system of record for purchases/subscriptions. The **AI Builders Workshop** is no
...
All Troy sends carry his persona signature block, appended inside
`sendTroyEmail` (`troySignature()`, 2026-08-06 signature-everywhere round);
the block contains no command syntax, so a quoted signature in an admin
reply can never execute either.
...
stored or written to disk): jszip + capped streaming inflate (2 MB/entry, 2000
entries), path normalization (jszip itself also strips `../` on load, verified
...
(`notify.ts`, governance `sendTroyEmail`; every Troy send is signed with
`troySignature()` at the seam). ~8 calls/run, worst case 10;
...
projections via `view.ts`; also runs the bounded opportunistic
...
missing/not-owned). `POST .../retry` — re-kick for received/failed/stale, all
...
UNCHANGED (no fromHeld) — kill switch, deploy marker, brain health, both
budget ledgers, one-panel-at-a-time serialization, per-row 3-runs/day claim
...
(2400 internal + 600 company brain calls/day) — the drain raises no cap. **Retention:** published rows live until deleted (they ARE the page
...
- **Signatures (owner directive 2026-08-06, superseding the 08-03 scoping:
  ...
  (per-call-site signing is how unsigned lanes shipped twice). Tron-from
  mail: `sendTronEmail` appends `tronSignature()` via `withTronSignature()`,
  so reject(), the acceptance receipt AND `warnAdmin` are all signed (the
  first two are byte-unchanged from the per-site era). Troy-from mail:
  governance `sendTroyEmail` appends `troySignature()` via
  `withTroySignature()`, covering every notify.ts lifecycle notice, the
  admin delete notice, the approval loop, and budget alerts. `sendRoadmapEmail`
  (§5.18) likewise signs at its seam via `withTronSignature()`. Two senders
  bypass the seams and sign at the call site: `sendArchiveRetentionEmail`
  (raw fetch, Troy) and the retention-failure WARN through module
  `sendEmail` (From = `oversight.mailFrom` = Tron; the module's contract is
  ...
  Call or Text" / baseUrl / memory-disclosure line). `troySignature()` in
  the same file is HOST-ONLY (the module has no Troy code, so no mirror and
  no source-hash pin): 4 lines (name / "AI Agent, XL.net AI" /
  Troy.Netter@ai.xl.net / baseUrl), deliberately WITHOUT the phone line
  ((872) 350-4325 is Tron's own AI voice line; a caller there reaches Tron)
  and WITHOUT the memory-disclosure line (Troy's lanes keep no conversation
  memory, the first-person claim would be false). `TROY_FROM` lives there
  too; budget.ts re-exports it. KEEP IN SYNC on module upgrades: `npm run
  test:work` pins BOTH the tron mirror's exact output AND a sha256 of the
  module function's source, so drift on either side fails the suite
  (re-sync, then re-pin); it also pins Troy's rendered block, the wrappers'
  idempotence, and seam wiring via source-scrape assertions.
...
- **Every failure reply is also an OPEN ISSUE (2026-08-05, owner directive:
  ...
  **Keys are EPISODIC, never per message** (refutation-round finding): a
  ...
  `governance:troy-dropped:<reason>`, and a single
  `governance:budget-reply:no-command`. `ledgerReasonSlug()` derives the
...
role. Six steps: 01 AI Governance doc on file, 02 Company Directory (Apollo
import + manual CRUD), 03 AI Builders Workshop (PAID, `$995`, links out to
`/builders#workshop`), 04 Submit AI-Built Work (the EXISTING §5.16 pipeline,
scoped per company; DKIM is its email lane's prerequisite), 05 Employee
Scorecard (derived, published-cards-only), 06 AI Builder Cohort (PAID,
`$495/mo`, links out to `/builders#cohort`). The two PAID steps are training
...
- `google` / `microsoft` — only with the per-login HMAC-covered `mv: true`
  ...
  `"true"` pass; `scripts/roadmap-tests.ts` pins it). Until the one-time
  Entra setup (optional claims `email` + `xms_edov` on the ID token) is
  done, ALL Microsoft sessions are roadmap-untrusted — sign-in still works
  everywhere; the portal offers the email-link lane. The claim is per-login,
  never a stored users-row flag (a stored flag would let a later forged
  login inherit an earlier genuine verification).
...
params), `requireGlobalAdmin` (isAdmin AND google AND exact xl.net —
...
the wrong provider gets the "sign in with Google" fix); the POST re-derives
...
work_submissions FIRST (the FK is RESTRICT by design), then the company row
...
disallows `/roadmap/` children); signed-in = the Lightline Runway (one
hairline through SIX diamond nodes, round-4 grammar: hollow = not started,
...
step 05 "Live" never "Done"; CSS only, reduced-motion kills it, state never
color-only, sr-only spans carry the phrase) + step cards, all fed from ONE
server-computed `roadmapStatus()` object. Steps are never hard-locked.
RUNWAY RULES FOR PAID STEPS: `UPNEXT_KEYS` (the four TRACKED steps) is the
single list, `isTracked()` derives from it, and the frontier ring can never
land on a step nobody can finish here. Paid stops are TRANSPARENT to segment
lighting: `reachedToward()` walks outward to the nearest TRACKED stop, so
segments 02-03 and 03-04 both light once directory AND work are reached, and
the tail 05-06 lights on `scorecard.live` (walking off the RIGHT end is
vacuously reached, deliberately: a permanently dark final segment on a fully
engaged company reads as breakage. Walking off the LEFT end returns false, so
a future reorder can never light a segment out of an unbought stop). The LINE
...
cannot know). SIX-STOP CSS MATH (roadmap.css lg block): 6 stops x 8rem + 5
segments x 2rem min-width = 928px hard minimum; `main` is max-w-7xl px-6, so
the 1024px breakpoint floor gives a 976px content box and the teaser wrap
widened max-w-4xl -> max-w-5xl to match the hub. MEASURED there: stops total
735.6px (two titles sit under the 8rem cap), so with 160px of segments and
both 40px caps the line lands at 975.6px in a 976px box and fits with the
caps uncompressed. 928px is the WORST case, if every title outgrew the cap;
the caps absorb that by shrinking, which is why they MUST keep `min-width:
0`. Verified at 1024/1030/1104/1280/1440 with no page overflow.
...
of the no-ledger reversal, owner-approved 2026-08-04), 02 directory (Apollo
import + manual CRUD; suppression fingerprints; import disclosure), 03 work
(shared submit surface + email card + own submissions + admin metadata list
+ company cards via the shared card template), 04 scorecard (ratio hero +
standing disclosure header; published cards only; zeros text-faint;
email-less rows annotated; not-in-directory submitters flagged with
admin-only add). Downloads (`GET /api/roadmap/docs/[id]`): principal-scoped
...
/api/roadmap/apollo-import`, company-admin only): host calls
`POST api/v1/mixed_people/search` directly (`q_organization_domains_list`,
x-api-key: APOLLO_API_KEY; the module's outreach sourcing stays disabled).
Persists EXACTLY {name, email, phone} + apollo_id (raw response never
persisted or logged; locked-email placeholders → null); upsert on
(company_id, apollo_id); manual rows never clobbered; suppressed sha256
emails skipped and counted. Page cap 5/run, fail-fast on any non-OK page
(no 429 retry — the 2/h/company limiter is also the double-click fence),
partial rows KEPT + reported, `apollo_last_import_*` stamped only on
complete runs, `roadmap_usage.apollo_calls` vs APOLLO_DAILY_CALL_CAP.
Admin-facing copy never names env vars.
...
strict emailDomain, scope mapping). Owner notifications: company created
...
gate: (1) STAFF (`isStaffSession` = google + exact xl.net, mv NOT required)
renders an explainer panel with /work + conditional /admin/roadmap links;
...
gate). Checked BEFORE the trusted check AND the principal path. (2) CLIENT
Google sessions without mv get ONE silent re-verify: the hub 302s through
...
builds a Google authorize URL with prompt=none + login_hint and the
module's state cookies. The hardened callback: (a) contained-error branch
...
non-binding; without this a browser signed into a different Google account
would be silently identity-swapped); (c) aix_rv deleted ONLY when mv was
minted; success-without-mv keeps the guard and appends ?verify=
google_unverified so the confirm screen can explain. (3) The confirm screen
...
dead control), and Microsoft stays interactive-only until the Entra
optional claims exist (SILENT_REVERIFY_PROVIDERS is google-only in code
with the three enable gates documented).
...
### 6.y Your AI Roadmap tables (migration 0035)
...
| table | purpose |
|---|---|
| `companies` | one row per client company, keyed by verified email domain (UNIQUE = the bootstrap race arbiter). CHECK `companies_domain_ck`: lowercase, never xl.net/ai.xl.net, never `%.onmicrosoft.com` |
...
| `company_people` | directory: exactly name/email/phone + apollo_id; partial UNIQUEs (company_id, lower(email)) and (company_id, apollo_id); source flips apollo→manual on human edit |
| `directory_suppressions` | sha256 of removed Apollo emails so deletion survives re-import (the PII itself is not retained) |
| `company_governance_docs` | step-1 documents: upload originals (bytea) or Governance Builder SNAPSHOTS (markdown copy, inert project id, no FK) |
