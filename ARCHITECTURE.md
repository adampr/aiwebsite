...
| `/work` | static server component | "Our Work" showcase: manifesto strip, then seven anchored product exhibits in narrative order (`#brain` Software Brain → `#aicompany` @aicompany/core → `#aiwebsite` this site, framed around the §1 oversight invariants → `#itsupportchicago` the autonomy experiment, explicitly "designed as a test of a 100% autonomous organization", sandbox facts first → `#qbr-machine` the Claude Code client-delivery pipeline (in production; three-deliverable sub-grid: Gap Analysis / Asset Strategy / QBR Deck; XL Lakehouse access layer) → `#roleplay` → `#leo-netter` internal Slack-bot test), grouped by kicker labels (Engine / What It Runs / Client Delivery / What We're Testing), mid-page + closing CTAs → `/builders` |
